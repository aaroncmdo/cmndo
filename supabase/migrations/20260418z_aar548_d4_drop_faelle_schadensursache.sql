...
-- Regel-#14-Daten-Check: beide Spalten (schadens_ursache + schadensursache)
-- 0 Zeilen befüllt in 14 Faellen — kein Backfill noetig.
-- Codebase nutzt 30+ Mal `schadens_ursache` (passt zum schadens_*-Pattern auf
-- faelle). `leads.schadensursache` bleibt unveraendert (eigene Tabelle).
-- KFZ-140-Sync-Trigger wird ebenfalls gedropt — er wird obsolet.
-- v_faelle_mit_aktuellem_termin haengt via `SELECT f.*` an der Spalte —
