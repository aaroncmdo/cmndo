-- #updates-rebuild Phase 4: dispatch+admin sehen alle Rueckrufe + alle offenen Leads.
--  * offener_rueckruf jetzt fuer dispatch UND admin (Oversight).
--  * NEU offener_lead: offene Leads als modus='info' (Sichtbarkeit im Verlauf, treibt
--    NICHT den Badge -> kein Bloat bei 172 offenen Leads). Spiegelt /dispatch/leads
--    (flow nicht abgeschlossen, nicht disqualifiziert/umgewandelt). dispatch+admin.
-- Die modus-Spalte war von Anfang an im Vertrag; getUpdates respektiert sie ab jetzt.
...
AS $$
  ...
  UNION ALL
  -- offener_rueckruf (dispatch+admin): offene Callback-Termine = Action
  ...
  UNION ALL
  -- offener_lead (dispatch+admin): offene Leads als INFO (Sichtbarkeit, KEIN Badge-Bloat)
  ...
  WHERE me.rolle IN ('dispatch','admin')
    ...
    AND l.status::text NOT IN ('disqualifiziert','umgewandelt','umgewandelt-sv');
$$;
