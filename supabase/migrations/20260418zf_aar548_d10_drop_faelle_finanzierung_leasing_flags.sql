...
--
-- Truth ist jetzt nur noch finanzierung_leasing-Enum
-- ('keine' | 'leasing' | 'finanzierung').
--
-- Regel-#14-Daten-Check:
--   - 14/14 Rows: beide Booleans = false + finanzierung_leasing = 'keine'
--   - Kein Backfill nötig (keine divergenten Werte)
--
-- Code-Sweep (siehe Commit):
--   - admin/dispatch/actions.ts: convertLeadToFaelle schreibt nur noch finanzierung_leasing
--   - ai-actions.ts, FallakteVollClient, StammdatenCard, briefing-prompt: lesen aus finanzierung_leasing
--   - seed-testdata: finanzierung_leasing statt leasing_flag
--   - lead-fall-mapping: Default-Einträge entfernt
--
-- leads-Tabelle behält beide Booleans (separate Scope — AAR-555).
...
COMMENT ON COLUMN faelle.finanzierung_leasing IS
  'Enum: keine | leasing | finanzierung. Source-of-Truth für Fahrzeug-Besitzverhältnis. '
