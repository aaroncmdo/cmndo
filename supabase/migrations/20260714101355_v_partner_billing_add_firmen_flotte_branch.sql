-- #8 Flotten-Money-Pfad, letzte Luecke: v_partner_billing blendet firmen_flotte komplett aus.
-- Die View ist eine UNION aus pro-Typ hardcodierten Branches (WHERE pp.partner_typ='makler'/'werkstatt');
-- partner_typ='firmen_flotte'-Rows (seit Mig 20260714094208) sind dadurch in der GESAMTEN Admin-Finance-UI
-- unsichtbar (getPartnerBilling -> PartnerBillingPanel).
--
-- Fix: EIN zusaetzlicher UNION-Branch, byte-identisch zum werkstatt-Branch, nur:
--   'werkstatt' -> 'firmen_flotte',  werkstaetten w -> firmen f,  w.name -> f.name,  w.ist_kleinunternehmer -> f.*
-- Empfaenger ist die FIRMA (partner_id = firmen.id, Aaron 14.07.); firmen.ist_kleinunternehmer existiert
-- seit Mig 20260714094208. Rest der View byte-identisch zur Live-Def (Basis: Mig 20260709220213 + Live-Verify).
-- Shape unveraendert (gleiche Spalten-Liste je Branch) => 0 Consumer-Impact ausser: firmen_flotte wird sichtbar.
