-- B4-slice-2a-ii Schritt B: die m_sub-claims.status-Terminal-/Regulierungs-Reads DROPPEN.
-- operative_status trägt jetzt JEDEN Terminal (endzustand B2 + Klage-Konvergenz + termin_durchgefuehrt)
-- und die Non-Terminals (slice-1b) -> o_sub (Schritt A) ist die Quelle, gewinnt den SUB_ORDER (o_ord15
-- > m_ord der Rest-Kaskade). m_sub liest claims.status NICHT mehr. (Der selbstzahler-Zweig-status-Read
-- bleibt vorerst — redundant zum op-Check, ohne selbstzahler-Seed nicht parity-abgedeckt; T3-Follow-up.)
-- Rest VERBATIM aus pg_get_viewdef (o_sub = Schritt-A-Stand); NUR die m_sub-status-WHEN-Zweige entfernt.
