-- Phase 3a: die 2 Alt-Tabellen-Dependencies auf partner_provisionen umstellen (verhaltensneutral;
-- Alt-Tabellen existieren noch, halten via dual-write dieselben Daten) — Prep fuer den Drop (3b/3c).
-- provision_betrag_netto MUSS numeric(10,2) bleiben (CREATE OR REPLACE VIEW verbietet Typ-Change;
-- partner_provisionen.betrag_netto_eur ist plain numeric) -> Cast.
...
-- delete_fall_komplett: makler_provisionen -> partner_provisionen (faengt beide partner_typ;
-- die alten Delete-Loops sind ohnehin EXCEPTION-gewrappt). partner_provisionen hat fall_id+claim_id.
...
DECLARE
  ...
  v_fall_tables text[] := ARRAY[
    ...
    'partner_provisionen','timeline','tasks','nachrichten','dokumente','fall_dokumente',
    'termine','flow_links'
  ];
