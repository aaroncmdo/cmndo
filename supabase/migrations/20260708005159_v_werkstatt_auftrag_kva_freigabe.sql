-- KVA-in-Auftrag (Aaron 07.07.: „die Werkstatt stellt als erstes immer einen Kostenvoranschlag aus").
-- Haengt 3 Spalten ANS ENDE von v_werkstatt_auftrag (CREATE OR REPLACE = nur Append erlaubt):
--   kostenvoranschlag_netto/brutto (claims.* — Werkstatt-KVA-Snapshot, NICHT der SV-Gutachten-Wert)
--   reparatur_freigegeben_am (claims.* — Kunde/Admin hat den KVA freigegeben)
-- So kann der Werkstatt-Auftrag den KVA-Status fuehren: benoetigt -> erstellt(EUR) -> freigegeben.
-- Gate (is_staff() OR is_werkstatt_for_claim) + voller bestehender Def unveraendert erhalten.
