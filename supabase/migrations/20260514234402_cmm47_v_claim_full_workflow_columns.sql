-- CMM-47 (2026-05-15) — v_claim_full um faelle-Workflow-Spalten erweitern
--
-- Ziel: Reader, die aktuell von `faelle` lesen (kundenbetreuer_id +
-- Workflow-Filter wie `status = 'abgeschlossen'`), auf `v_claim_full`
-- migrieren können — Vorbereitung für Phase-4 DROP der Duplikat-Spalten.
--
-- Problem: claims.status ≠ faelle.status (claim-Lifecycle vs. fall-Workflow,
-- unterschiedliche Enums — bewusst nicht via Sync-Trigger gesynct, siehe
-- 20260505134954). v_claim_full hat bisher nur c.status, also greift
-- `WHERE status = 'abgeschlossen'` auf claims.status zu — semantisch falsch
-- für KB-Performance-Pages.
--
-- Lösung: f.status zusätzlich als fall_status in die View aufnehmen + alle
-- Cron-relevanten faelle-Workflow-Spalten gleich mit, damit CMM-47 Cluster
-- A.1 (Team-Pages) + A.2 (Cron-Routes) jeweils in eigenen Code-PRs ohne
-- weitere View-Migration migriert werden können.
--
-- Hinzugefügte Spalten (alle aus faelle):
--   fall_status, fall_created_at, fall_updated_at, kundenbetreuer_fallback_flag,
--   aktuelle_phase, szenario, dokumente_vollstaendig_fuer_phase,
--   dokumente_reminder_whatsapp_letzte_sendung, no_show_gemeldet_am,
--   re_termin_token, sa_unterschrieben_am, vollmacht_signiert_am, mandatsnummer
--
-- Pattern: DROP + CREATE (Postgres REPLACE schlägt fehl bei Schema-Erweiterung
-- mit neuen Spalten am Ende — siehe Lesson Learned 2 in CMM-Memory).
...
SELECT
  ...
  f.sv_id,
  f.service_typ,
  -- CMM-47 neue faelle-Workflow-Spalten
