-- CMM-44 MP-8: v_claim_phase spiegelt getClaimLifecycle (lib/claims/lifecycle.ts).
-- Ergaenzt +2 terminale Substates (abgelehnt_final, an_externe_kanzlei) und die
-- Status-getriebene Regulierung (in_kommunikation_vs/abgelehnt) NACH dem lexdrive-Zweig.
-- Prioritaet: abschluss > regulierung(lexdrive) > regulierung(status) >
-- Kanzlei-Interim > begutachtung > erfassung. CREATE OR REPLACE = Spalten unveraendert
-- (claim_id/main_phase/sub_phase), Grants + Optionen bleiben erhalten.
-- Appliziert via Supabase-Plugin apply_migration, recorded version 20260529102822.
