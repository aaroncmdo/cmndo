-- AV3/AV4/AV5 (abrechnungsweg-abhaengige Auftragssicht): neue KVA-Reparaturdauer-Spalte +
-- v_werkstatt_auftrag um unfallart (Auffahrunfall-Hinweis), reparaturdauer_tage (Gutachten) und
-- reparaturdauer_tage_kva (Werkstatt-KVA) erweitern. Append-only (bestehende Spalten unveraendert);
-- die gu-LATERAL-Subquery bekommt zusaetzlich g.wiederbeschaffungsdauer_tage.
ALTER TABLE public.claims ADD COLUMN IF NOT EXISTS reparaturdauer_tage_kva integer;
