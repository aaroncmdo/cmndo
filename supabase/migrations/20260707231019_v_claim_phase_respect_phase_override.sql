-- Phase 1d: v_claim_phase respects claims.phase_override (canonical override for the
-- consumers that read v_claim_phase directly, e.g. KanbanBoardClient / getClaimPhaseMap).
-- EXACT existing derivation preserved (9 distinctive substrings verified intact); only
-- additions: outer LEFT JOIN claims co + COALESCE(co.phase_override, <main_phase CASE>).
-- phase_override is all-NULL -> output provably unchanged until an override is set.
-- Gate claim_sichtbar_fuer_aktuellen_user + authenticated grant preserved (audit=0 verified).
-- NOTE: v_claim_full inline-duplicates this derivation (does NOT read v_claim_phase), so the
-- cockpit override is additionally applied in v_claim_workstate; full canonical propagation
-- to v_claim_full/v_claim_base requires de-duplicating the derivation (follow-up).
