-- KVA-Ablehnung-Loop (Werkstatt-Reparatur): v_werkstatt_auftrag exponiert kva_abgelehnt_am +
-- kva_abgelehnt_grund, damit die Werkstatt-Sicht (kvaStatus) den 'abgelehnt'-State erkennt
-- (statt faelschlich 'erstellt'/"wartet auf Freigabe") und einen Re-Upload anbietet. Bisher
-- deferred (#4745 R1). Append-only: die 2 Spalten kommen ans ENDE (nach
-- reparatur_rueckruf_wunschzeit), sonst bricht CREATE OR REPLACE. Keine security_invoker-Option
-- (reloptions=null, Owner postgres) -> unveraendert. Types-Regen aufgeschoben: AUFTRAG_SELECT
-- liest die Spalten ueber einen untypisierten string, der WerkstattAuftrag-Typ ist manuell.
