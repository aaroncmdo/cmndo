-- RLS-Haertung Schritt 2b (Erweiterung): Identity-Cross-Compare auf ALLE 8 Rollen (vorher kunde/sv/kb).
-- + kanzlei (Gate: rolle=kanzlei AND service_typ=komplett -> X=komplett, Y=non-komplett)
-- + makler  (Gate: makler_id ODER consent -> X=verlinkt, Y=unverlinkt; cccc5555 hat consent = legit X!)
-- + werkstatt (Gate: werkstatt_id-in-eigene-werkstaetten -> X=verlinkt, Y=unverlinkt)
-- + admin/dispatch (Gate: rolle in admin/dispatch -> sehen ALLE -> nur POSITIV, Y=NULL; X=Kunde-Claim
--   = sehen einen Claim ZU DEM SIE KEINE direkte Linkage haben = beweist Breit-Zugriff).
-- leadbearbeiter == dispatch (redundant, ausgelassen). X/Y roh aus Linkage (nicht zirkulaer).
...
BEGIN
  ...
  SELECT c.id INTO x_kanzlei FROM claims c WHERE c.service_typ='komplett'
    AND c.geschaedigter_user_id IS DISTINCT FROM u_kanzlei LIMIT 1;
  SELECT c.id INTO y_kanzlei FROM claims c WHERE c.service_typ IS DISTINCT FROM 'komplett'
