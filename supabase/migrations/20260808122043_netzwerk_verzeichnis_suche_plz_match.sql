-- Profi-Verzeichnis: PLZ-Suche (Aaron-Befund 08.08. "findet immer keine Treffer" --
-- das Feld verspricht "Ort", aber die Suche matchte nur Namen+Ortsnamen; eine PLZ
-- lieferte deterministisch 0 Treffer). Neu: PLZ-Praefix-Match ueber die
-- rollen-disjunkten Satelliten-PLZs + PLZ in der Ort-Anzeige ("50667 Koeln").
    ...
    select p.id,
     ...
     where p.rolle::text in ('sachverstaendiger','werkstatt','flottenmanager')  -- Knoten-Rollen (Makler v1 kein Ziel)
       ...
       and (
         ...
         or coalesce(wk.adresse_plz, sv.standort_plz, ff_firma.adresse_plz, p.plz, '') ilike q||'%'
       )
