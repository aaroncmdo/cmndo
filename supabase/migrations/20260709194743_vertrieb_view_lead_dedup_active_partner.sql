-- Vertrieb-CRM Dedup: konvertierte + bereits-aktive Leads aus der vereinten View verdraengen.
-- sv-lead: raus wenn konvertiert ODER die Firma bereits als aktiver sachverstaendige existiert
--          (firmenname-Match faengt Selbstregistrierung ab, die konvertiert_zu_sv_id NICHT setzt).
-- partner-lead: raus wenn zu Makler/Werkstatt/User konvertiert (erscheint dann als aktiver Partner).
-- Karte/getDeadPins bleiben unberuehrt (Filialen dort erwuenscht). security_invoker + Grants unveraendert.
...
with (security_invoker = true) as
  ...
  select sl.id, 'sv-lead'::text,
   ...
   where sl.konvertiert_zu_sv_id is null
     and not exists (
       select 1 from public.sachverstaendige sx
        where sx.geloescht_am is null
          and nullif(trim(sl.firma), '') is not null
          and lower(trim(sx.firmenname)) = lower(trim(sl.firma))
     )
  union all
