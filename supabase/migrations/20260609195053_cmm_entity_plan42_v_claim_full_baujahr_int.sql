-- CMM-Entity Plan 4.2: fahrzeug_baujahr date -> integer (Jahr) in v_claim_full (CMM-49-Ask #2).
-- Type-Change erfordert DROP+recreate (CREATE OR REPLACE kann Spaltentyp nicht aendern).
-- v_claim_full ist LEAF (0 DB-Dependents verifiziert) -> DROP contained, kein Cascade.
-- SICHERHEIT: public.pg_default_acl grantet anon automatisch ALL auf neue Views (PII-Leak-Klasse,
--   da SECURITY DEFINER + RLS-Bypass) -> nach CREATE ZWINGEND REVOKE anon + GRANT authenticated/service_role
--   (restauriert exakt den Pre-Drop-Grant-Zustand). reloptions bleibt NULL = definer (kein security_invoker).
-- baujahr-Spalte hatte 0 Consumer (erst in Plan 4 angelegt) -> kein Reader bricht.
