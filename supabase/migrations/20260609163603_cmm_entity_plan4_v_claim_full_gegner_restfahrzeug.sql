-- CMM-Entity Plan 4: v_claim_full Gegner entity-sourced (§7) + Restfahrzeug-Detail (veh) + vorschaden_erkannt (claims).
-- Additiv: 16 neue Spalten ans Ende + gegner_fahrzeugtyp von NULL->COALESCE(gveh.bauart, gp.fahrzeugtyp_klartext).
-- security_invoker NICHT gesetzt (Bestand reloptions=NULL = definer; bewusst erhalten -> kein WITH-Clause).
-- Value-preserving verifiziert (existing_diff=0 vs Live-View, 79 Rows). Entsperrt CMM-49 EMBED-Reader-Repoint.
-- ist_fahrzeughalter bewusst NICHT enthalten (keine value-preserving entity-Quelle; Follow-up).
