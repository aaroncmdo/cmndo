-- B4-slice-2a-ii Schritt A (additiv, No-Op): v_claim_phase.o_sub um die feinen Terminals
-- ergaenzen (reguliert_vollstaendig/klage_rechtsstreit/verjaehrt/abgelehnt_final/
-- an_externe_kanzlei_uebergeben/termin_durchgefuehrt). Behavior-preserving: solange m_sub die
-- status-Terminals noch liest, gewinnt der SUB_ORDER-Tie (o_ord=m_ord=15) ohnehin m_sub mit
-- demselben Wert. Schritt B (Folge-Migration) droppt dann die m_sub-status-Reads.
-- Rest der View VERBATIM aus pg_get_viewdef (nur die 6 o_sub-WHEN-Zweige neu vor ELSE NULL).
CREATE OR REPLACE VIEW public.v_claim_phase AS
 SELECT fw.claim_id,
        CASE
            ...
            CASE
                WHEN (co.status = ANY (ARRAY['reguliert_vollstaendig'::text, 'storniert'::text])) OR (co.operative_status = ANY (ARRAY['abgeschlossen'::text, 'storniert'::text])) THEN 'abschluss'::text
        ...
        CASE
            WHEN derive_abrechnungsweg(co.service_typ, lo.schuldfrage, lo.eigene_versicherung, co.schadenart) = 'selbstzahler'::text THEN
            CASE
                WHEN co.status = 'reguliert_vollstaendig'::text OR co.operative_status = 'abgeschlossen'::text THEN 'erfolgreich_reguliert'::text
                WHEN co.status = 'storniert'::text OR co.operative_status = 'storniert'::text THEN 'storniert'::text
