-- CMM-Entity Plan 4.4: v_claim_full Scope-Luecke + gegner_anzahl_beteiligte-Fix (CMM-49 Flags).
-- Flag 1: gegner_anzahl_beteiligte war NULL-Platzhalter -> (anzahl_beteiligte_total - 1) (claims-Kanon).
-- Flag 2: 4 fehlende Reader-Felder additiv: fahrzeug_fahrbereit/fahrzeugschaden_beschreibung (claims),
--   fahrzeug_aufbau (vehicles.aufbau), gegner_versicherung (COALESCE entity-name + verursacher-klartext).
-- gegner_versicherung_anfrage_datum = bewusster Verzicht (Aaron 10.06.). faelle leer (greenfield) -> LOSS=0.
-- Additiv (4 Spalten + 1 LATERAL-Feld + 1 Expr-Fix) -> CREATE OR REPLACE (Grants/anon-Revoke erhalten).
