-- AAR-Stufe-0.5 Punkt 1 (14.05.2026)
-- Drop von claims.firma_name nach Trigger-Patch.
--
-- Vorab-Verifikation:
--   - claims.firma_name: 0/11 Coverage, kein Writer, kein UI-Reader
--   - SSoT fuer Firma-Daten: claim_parties.firma + .ist_gewerbe + .ust_id
--   - faelle.firma_name bleibt drin (dispatch-fall-actions schreibt es
--     aus lead.firma_name) — Legacy-Pfad bis CMM komplett claims-zentriert
--   - v_claim_full View referenziert firma_name NICHT (geprueft via
--     pg_get_viewdef nach claims-Stufe-0)
--
-- Patch: Beide Sync-Trigger-Funktionen ohne firma_name-Sync neu definieren.

BEGIN;

-- 1. sync_claims_to_faelle ohne firma_name-Zeile
...
-- 2. sync_faelle_to_claims ohne firma_name-Zeile
...
-- 3. Trigger neu erzeugen ohne firma_name in der UPDATE OF-Spaltenliste
...
-- trg_sync_faelle_to_claims bleibt unveraendert (faelle.firma_name existiert
-- weiter; Trigger feuert weiter bei faelle.firma_name-Updates, aber
-- sync_faelle_to_claims schreibt jetzt keine firma_name auf claims mehr — sauber).

-- 4. claims.firma_name droppen (jetzt ohne abhaengigen claims-Trigger)
