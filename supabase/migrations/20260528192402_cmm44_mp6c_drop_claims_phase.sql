-- 1) Drop the 6 dependent views (pg_depend closure = exactly these 6; verified live).
...
-- 2) Drop the phase trigger-web (3 triggers + functions).
...
-- live signature is (text, text, uuid), not zero-arg; use exact sig so IF EXISTS actually matches.
drop function if exists public.map_claim_phase_to_faelle_phase(text, text, uuid);
drop function if exists public.calc_claims_phase(uuid, text, uuid);

-- 3) Drop the orphan sync fn (no trigger uses it; references phase + the map fn).
drop function if exists public.trg_fn_sync_claims_to_faelle();

-- 4) Rewrite the live daily cron (jobid 14, '30 9 * * *') off claims.phase onto v_claim_phase.
...
-- 5) Drop the column (irreversible — 0 readers post-6a, derived-only value).
alter table public.claims drop column phase;

-- 6) Recreate the 6 views WITHOUT phase, restore GRANTs.
...
create view public.v_faelle_mit_aktuellem_termin as
 SELECT f.id,
   ...
   FROM faelle f
     ...
     LEFT JOIN v_claim_phase vcp ON vcp.claim_id = c.id;
grant all on public.v_faelle_mit_aktuellem_termin to anon, authenticated, service_role;
 ...
 SELECT f.id,
   ...
   FROM faelle f
     ...
     LEFT JOIN LATERAL ( SELECT gt.besichtigungsort_adresse,
         ...
         LIMIT 1) spd_termin ON true
     LEFT JOIN v_claim_phase vcp ON vcp.claim_id = c.id;
grant all on public.faelle_kunde_view to anon, authenticated, service_role;
 ...
 SELECT f.id,
   ...
   FROM faelle f
     ...
     LEFT JOIN LATERAL ( SELECT gt.besichtigungsort_adresse,
         ...
         LIMIT 1) spd_termin ON true
     LEFT JOIN v_claim_phase vcp ON vcp.claim_id = c.id;
grant all on public.faelle_sv_view to anon, authenticated, service_role;
...
create view public.v_claim_sv with (security_invoker = false) as
 SELECT id,
    ...
    zeugen_kontakte
   FROM claims c
  WHERE is_sv_for_claim(id);
grant all on public.v_claim_sv to anon, authenticated, service_role;
