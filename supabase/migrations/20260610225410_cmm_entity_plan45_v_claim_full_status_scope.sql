-- CMM-Entity Plan 4.5: v_claim_full Status-Scope (CMM-49 Sweep-Flags Batch 2).
-- operative_status (claims-Kanon; fall_status war schon = c.operative_status, aber CMM-49 brauchts by-name),
-- spezifikation (claims), zeugen_vorhanden (claims). Alle additiv -> CREATE OR REPLACE (Grants erhalten).
-- kanzlei_uebergabe_am = Phantom (existiert nirgends -> flows.ts:380-Bug, NICHT hier). Verzicht: keine Aufnahme.
