-- §12-Schritt-2: Konfidenz-/Match-Funktion (read-only). Liefert Kandidaten + Score + Tier.
-- Signale nach §4-Tiers: HART = verifizierter Kontakt (verified_contacts), STARK = Name+Geburtsdatum,
-- WEICH = getippte Felder (personen.email/telefon/mobil) bzw. Name allein.
-- FIN (§13-C) + airdrop (§13-B) bewusst NOCH NICHT — vehicles/Owner-Linkage erst forward-looking (1 FIN-Row).
-- SECURITY DEFINER + execute nur service_role (liest PII verified_contacts/personen => kein Client-Enumeration).
