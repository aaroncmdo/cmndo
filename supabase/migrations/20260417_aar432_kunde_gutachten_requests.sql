-- AAR-432 (Child 3 von AAR-429): Opt-in Gutachten-Weiterleitung an Kunde
-- via zeitlich begrenztem Magic-Link. Tabelle dient Audit-Trail +
-- Rate-Limiting (max 3 Anfragen pro Fall in 24h).
...
-- Default: keine Policies → nur Service-Role (bypasses RLS) darf lesen/schreiben.
-- Kein SELECT/INSERT/UPDATE/DELETE für anon oder authenticated.
