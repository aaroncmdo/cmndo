-- Vertrieb-CRM P0 Task 3: vereinende v_vertrieb_kontakt-View über die 5 Partner-Silos.
-- security_invoker=true (KEIN DEFINER → nicht von audit_ungated_definer_views geflaggt);
-- gelesen NUR via createAdminClient (service_role) nach Staff-Role-Guard.
-- lat/lng: sachverstaendige=numeric → ::double precision (UNION-Parität mit den anderen).
...
with (security_invoker = true) as
  ...
  select l.id, 'partner-lead'::text,
         ...
         l.status, null::boolean, false, null::boolean, null::boolean, null::boolean, null::text
  ...
  select sl.id, 'sv-lead'::text,
         ...
         coalesce(sl.warteliste_status, sl.claim_status)
  ...
  select m.id, 'makler'::text,
         ...
         (m.onboarding_abgeschlossen is not true), null::text
  ...
  select w.id, 'werkstatt'::text,
         ...
         w.status, null::boolean, (w.gesperrt_am is not null), null::boolean, null::boolean, false, null::text
