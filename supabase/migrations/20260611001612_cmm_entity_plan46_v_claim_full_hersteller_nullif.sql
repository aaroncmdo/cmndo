-- CMM-Entity Plan 4.6: fahrzeug_hersteller normalisieren — NULLIF(veh.hersteller, 'Unbekannt').
-- 1 vehicle (FIN WVWZZZ1KZAW000000, lead_konvertierung) traegt Placeholder-Literal 'Unbekannt' (= Unknown
-- = semantisch NULL); faelle.fahrzeug_hersteller ist dort NULL -> Repoint waere sonst leer->'Unbekannt' (Flip).
-- faelle haelt NIE 'Unbekannt' (0 Rows) -> NULLIF = 0 LOSS. Durable: Converter (ensureVehicleFromFin) kopiert
-- lead.fahrzeug_hersteller verbatim -> Funnel-Fallback 'Unbekannt' kann wiederkehren -> View-Normalisierung statt Data-Fix.
-- Additiv (1 Expr-Change) -> CREATE OR REPLACE (Grants/anon-Revoke erhalten).
CREATE OR REPLACE VIEW public.v_claim_full AS
  SELECT c.id, c.vehicle_id, c.schadentag, c.schadenzeit, c.entdeckt_am, c.schadenort_adresse, c.schadenort_plz, c.schadenort_ort, c.schadenort_land, c.schadenort_lat, c.schadenort_lng, c.schadenort_kategorie, c.hergang_kunde_text, c.hergang_sv_text, c.schadenart, c.fall_typ, c.unfall_konstellation, c.fahrerflucht, c.auslandskennzeichen, c.polizei_aktenzeichen, c.polizei_bericht_vorhanden, c.polizei_vor_ort, c.polizeibericht_status, c.geschaedigter_user_id, c.gegnerisches_vehicle_id, c.gegner_versicherung_id, c.gegner_versicherungsnummer, c.gegner_aktenzeichen, c.gegner_bekannt, c.anzahl_beteiligte_total, c.hat_personenschaden, c.hat_mietwagen, c.hat_nutzungsausfall, c.hat_sachschaden, c.hat_abschleppung, c.sachschaden_beschreibung, c.halter_ungleich_fahrer, c.kunden_konstellation, c.unfallskizze_url, c.unfallskizze_svg, c.unfallskizze_bestaetigt, c.unfallskizze_ablehnung_grund, c.unfallskizze_generiert_am, c.status, c.abgeschlossen_am, c.verjaehrt_am, c.created_at, c.updated_at, c.created_by_user_id, c.created_via, c.claim_nummer, c.lead_id, c.kundenbetreuer_id, c.vs_ablehnungs_grund, c.regulierungs_betrag, c.endzustand_gesetzt_durch_user_id, c.endzustand_gesetzt_am, c.endzustand_grund, c.kanzlei_wunsch, c.kanzlei_wunsch_gefragt_am, c.kanzlei_wunsch_gefragt_in_phase,
    ...
    veh.kennzeichen_aktuell::text AS kennzeichen, NULLIF(veh.hersteller, 'Unbekannt') AS fahrzeug_hersteller, veh.modell_haupttyp AS fahrzeug_modell, veh.bauart AS fahrzeug_typ,
    ...
    (c.anzahl_beteiligte_total - 1)::integer AS gegner_anzahl_beteiligte,
    ...
    c.fahrzeug_fahrbereit, c.fahrzeugschaden_beschreibung, veh.aufbau AS fahrzeug_aufbau, COALESCE(gv.name, gp.versicherung_klartext) AS gegner_versicherung,
    c.operative_status AS operative_status, c.spezifikation, c.zeugen_vorhanden
  FROM claims c
    ...
    LEFT JOIN LATERAL (SELECT vp.firma_id, vp.person_id, vp.vehicle_id, vp.vorname, vp.nachname, vp.fahrzeugtyp_klartext, vp.kennzeichen, vp.versicherung_klartext FROM claim_parties vp WHERE vp.claim_id = c.id AND vp.rolle = 'verursacher' ORDER BY vp.reihenfolge, vp.created_at LIMIT 1) gp ON true
