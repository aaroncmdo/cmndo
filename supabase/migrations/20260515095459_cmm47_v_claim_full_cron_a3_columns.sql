-- CMM-47 (2026-05-15) Cluster A.3 — v_claim_full um Cron-Workflow-Spalten erweitern
--
-- Recovery-File: diese Migration wurde am 2026-05-15 09:54:59Z via Supabase-MCP
-- direkt auf die Prod-DB appliziert (siehe schema_migrations.version =
-- 20260515095459). Das File hier ist die nachgelegte Repo-Spur, damit
-- `supabase db reset` reproduzierbar bleibt — REGEL 2 (DDL-via-CLI) wurde
-- damals nicht eingehalten, dieser File-Backfill macht den Repo-Stand wieder
-- mit dem Live-Schema konsistent.
--
-- Ziel: nach CMM-47 Cluster A.1 (Team-Pages, Migration 20260514234402) jetzt
-- Cluster A.2 (7 Cron-Routes) ohne weitere View-Migration durchführbar machen.
-- Cron-Routes brauchen folgende faelle-Workflow-Spalten zusätzlich zu A.1:
--
...
--   vs_eskalationsstufe
--
-- Pattern: DROP + CREATE (Postgres REPLACE schlägt fehl bei Spalten-
-- Erweiterung mit neuen Spalten am Ende — Lesson Learned 2 in CMM-Memory).
...
SELECT
  ...
  f.sv_id,
  f.service_typ,
  -- CMM-47 Cluster A.1 (von 20260514234402)
  ...
  f.mandatsnummer,
  -- CMM-47 Cluster A.3 NEU (für Cron-Routes)
