-- CMM-44 MP-6b: additively expose main_phase + sub_phase on 5 views via v_claim_phase join.
-- Additive only: aktuelle_phase and every existing column left verbatim; two new columns
-- (vcp.main_phase, vcp.sub_phase) appended to the END of each SELECT list, joined via
-- LEFT JOIN public.v_claim_phase vcp ON vcp.claim_id = <claims-id-of-view>.
