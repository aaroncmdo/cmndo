-- CMM-44 SP-D: Repoint 4 views' termin columns to gutachter_termine (current/latest termin per claim)
-- instead of sourcing them from faelle. faelle columns stay (additive); source changes only.
-- Output column names are byte-identical (CREATE OR REPLACE VIEW).
-- migration repair --status applied <version> after apply.
--
-- Per-view summary:
--   v_faelle_mit_aktuellem_termin: extend existing t LATERAL (status-ordered) with 19 ADD cols;
--     update outer SELECT for all 21 SPD cols (19 same-name + 2 DUPs already in LATERAL).
--   faelle_kunde_view: add spd_termin LATERAL (ORDER BY start_zeit DESC); repoint 6 cols.
--   faelle_sv_view: add spd_termin LATERAL (ORDER BY start_zeit DESC); repoint 5 cols.
--   v_claim_full: add spd_termin LATERAL (ORDER BY start_zeit DESC); repoint 9 cols.

BEGIN;

-- ============================================================
-- Block 0: SP-D PR1 added besichtigungsort_lat/lng + geschaetzte_fahrdistanz_km
-- as plain `numeric`; faelle had precision (lat/lng numeric(10,7), fahrdistanz
-- numeric(6,1)). Align the gt column types so the repointed view outputs keep
-- their original precision (otherwise CREATE OR REPLACE fails with 42P16). Small
-- table, values backfilled from faelle so they fit. No view depends on these gt
-- columns yet (PR1 deferred the repoint), so the ALTER is safe.
-- ============================================================
ALTER TABLE public.gutachter_termine
  ALTER COLUMN besichtigungsort_lat TYPE numeric(10,7),
  ALTER COLUMN besichtigungsort_lng TYPE numeric(10,7),
  ALTER COLUMN geschaetzte_fahrdistanz_km TYPE numeric(6,1);

-- ============================================================
-- 1. v_faelle_mit_aktuellem_termin
--    Extend existing t LATERAL (status-priority ORDER) with 19 new SP-D cols.
--    Outer SELECT: replace f.<spd_col> with t.<spd_col>
--    DUPs: f.geschaetzte_fahrzeit_min -> t.geschaetzte_fahrtzeit_min AS geschaetzte_fahrzeit_min
--          f.gcal_event_id -> t.google_event_id AS gcal_event_id
-- ============================================================
...
-- ============================================================
-- 2. faelle_kunde_view
--    Add spd_termin LATERAL (ORDER BY start_zeit DESC NULLS LAST).
--    Repoint 6 SP-D cols:
...
--      no_show_gemeldet_am, re_termin_token,
--      re_termin_token_eingelaufen_am, re_termin_eskalation_an_kb_am
-- ============================================================
