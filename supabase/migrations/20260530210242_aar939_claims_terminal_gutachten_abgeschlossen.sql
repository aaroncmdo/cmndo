-- Terminal-Status gutachten_abgeschlossen (CMM-44 / AAR-939): nur_gutachter-Claims
-- (embed-B + nativ) schliessen nach dem Erstgutachten terminal ab — es gibt keine
-- Regulierung/Kanzlei. Bisher fehlte ein passender End-Status (Claim haengt in
-- 'begutachtung'). gutachten_abgeschlossen -> Hauptphase 'abschluss',
-- Subphase 'gutachten_abgeschlossen'. Parity-Pflicht: bitgleich zu lifecycle.ts
-- (ABSCHLUSS_SUBSTATE) + SUBPHASE_LABEL/ClaimSubPhase (im selben PR gesetzt).
...
ALTER TABLE public.claims ADD CONSTRAINT claims_status_check CHECK (
  status = ANY (ARRAY[
    ...
    'verjaehrt'::text, 'abgelehnt_final'::text,
    'gutachten_abgeschlossen'::text
  ])
);
...
CREATE OR REPLACE VIEW public.v_claim_phase WITH (security_invoker = true) AS
 SELECT c.id AS claim_id,
        CASE
            WHEN c.status = ANY (ARRAY['reguliert_vollstaendig'::text, 'storniert'::text, 'klage_rechtsstreit'::text, 'verjaehrt'::text, 'abgelehnt_final'::text, 'an_externe_kanzlei_uebergeben'::text, 'gutachten_abgeschlossen'::text]) THEN 'abschluss'::text
        ...
        CASE
            ...
            WHEN c.status = 'gutachten_abgeschlossen'::text THEN 'gutachten_abgeschlossen'::text
