-- B1 Tier3 — nachrichten: die letzte offene Tabelle des Advisor-Hygiene-Passes (313 -> 6 -> 0).
--
-- WARUM ERST JETZT: nachrichten wurde in Tier1/Tier2 (PR #4301) bewusst ausgelassen, weil die
-- Lane kitta/fix-direktthread-idor live an genau diesen Chat/Thread-Policies arbeitete — ein
-- name-basiertes DROP dort waere nach einem B1-Merge gebrochen. Diese Lane ist inzwischen
-- gelandet (PR #4295 "IDOR-Gate auf den authed Thread-Wrappern"): ein CODE-Gate auf den
-- Server-Wrappern, KEINE RLS-Migration. Die 6 permissiven Policies sind unveraendert ->
-- Kollisionsgefahr entfaellt.
--
-- RESTRICTIVE BLEIBT UNANGETASTET: nachrichten traegt zusaetzlich die RESTRICTIVE-Policy
-- `nachrichten_thread_insert_member_only` (INSERT, TO public). Der Block filtert durchgehend auf
-- permissive='PERMISSIVE' -> sie wird weder gemergt noch gedroppt. Postgres' Semantik
-- (OR der permissiven) AND (AND der restriktiven) bleibt damit exakt erhalten. Nachbedingung (4)
-- prueft das explizit.
--
-- Sonst identisch zu Tier1/Tier2: <=1 permissive Policy je (Rolle, Command), qual = OR jeder
-- original-anwendbaren Policy -> semantischer No-op by construction (Postgres OR-verknuepft
-- permissive Policies ohnehin). SELBST-VERIFIZIEREND & FAIL-CLOSED (Struktur + USING/CHECK-
-- Containment; Verstoss -> RAISE -> Abbruch, prod unveraendert).
--
-- EMPIRISCHER BEWEIS (nachrichten ist die einzige B1-Tabelle mit ECHTEN Daten, 153 Zeilen):
-- Rollen-Sichtbarkeit mit echten JWTs vorher == nachher, 8/8 Rollen identisch —
--   admin 153/153 · dispatch 153/153 · kanzlei/kundenbetreuer/makler/sv/werkstatt 0/0 ·
--   anon 42501/42501 (pre-existing: qual referenziert Tabellen ohne anon-Grant).
...
CREATE TEMP TABLE _b1_orig ON COMMIT DROP AS
  ...
  FROM pg_policies
  WHERE schemaname='public' AND permissive='PERMISSIVE' AND tablename = ANY(ARRAY['nachrichten']);
...
DO $b1$
DECLARE
  v_tables text[] := ARRAY['nachrichten'];
...
BEGIN
  FOREACH v_t IN ARRAY v_tables LOOP
    ...
    -- NUR permissive Originale droppen; die RESTRICTIVE-Policy bleibt bestehen.
...
BEGIN
  ...
  SELECT string_agg(tablename||'/'||rl||'/'||cx, ', ') INTO v_bad FROM (
    ...
    WHERE p.schemaname='public' AND p.tablename = ANY(ARRAY['nachrichten']) AND p.permissive='PERMISSIVE' AND rl IN ('anon','authenticated')
    GROUP BY 1,2,3 HAVING count(*)>1
  ) x;
  IF v_bad IS NOT NULL THEN RAISE EXCEPTION 'B1 STRUKTUR >1 Policy: %', v_bad; END IF;

  -- (2) USING-CONTAINMENT (whitespace/paren-normalisiert gegen Postgres-Redeparse)
  ...
  -- (4) RESTRICTIVE unangetastet (nachrichten_thread_insert_member_only muss ueberleben)
  IF (SELECT count(*) FROM pg_policies WHERE schemaname='public' AND tablename='nachrichten' AND permissive='RESTRICTIVE') <> 1 THEN
    RAISE EXCEPTION 'B1: RESTRICTIVE-Policy auf nachrichten verloren!';
  END IF;
END $v$;
