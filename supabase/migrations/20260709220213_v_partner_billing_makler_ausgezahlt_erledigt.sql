-- Fix: makler-Provisions-Branch normalisiert 'ausgezahlt' NICHT zu 'erledigt' (wie werkstatt) ->
-- eine per auszahlenProvision bezahlte Makler-Provision fiel auf ELSE status='ausgezahlt', wodurch
-- Gutschrift-Download (belegeFuerZeile) + Korrigieren-Button (status_norm='erledigt'-Gate) versteckt
-- blieben. EINE Zeile ergaenzt: `WHEN pp.ausgezahlt_am IS NOT NULL THEN 'erledigt'` im makler-Branch
-- (nach storniert, damit Storno gewinnt; abrechnung_id-Klausel bleibt fuer den Batch-Abrechnungs-Pfad).
-- Rest byte-identisch zur Live-Def. Blast-Radius verifiziert: nur partner-billing.ts liest die View,
-- keine externen status_norm-Consumer. werkstatt/staffel/maik unveraendert.
