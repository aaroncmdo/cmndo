-- SP3: v_werkstatt_auftrag um einen minimierten Gutachten-Satz erweitern (additiv).
-- Bestehende Definition (inkl. SP2-reparatur_termine-LATERAL) 1:1 uebernommen;
-- LEFT JOIN LATERAL auf das juengste FERTIGGESTELLTE Gutachten + 8 neue Spalten.
-- Gate unveraendert (is_staff() OR is_werkstatt_for_claim). Die Werkstatt bekommt
-- reparatur-relevante OCR-Werte + den PDF-Pfad (Letzterer NUR server-seitig ueber
-- die signed-URL-Action, nicht im Client gerendert).
