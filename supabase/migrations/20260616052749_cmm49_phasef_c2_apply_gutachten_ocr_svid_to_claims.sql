-- CMM-49 Phase F (Batch C2): apply_gutachten_ocr liest sv_id aus claims (SSoT) statt faelle.
-- Einzige faelle-Zeile der Funktion; value-neutral (claims.sv_id == faelle.sv_id, div=0 live verifiziert).
-- Damit ist apply_gutachten_ocr der LETZTE DB-interne faelle.sv_id-Reader, der faelle-frei wird.
-- Der claims->faelle-Sync (sync_claims_sv_id_to_faelle) bleibt absichtlich bestehen, bis die
-- faelle.sv_id-CODE-Writer (admin/_karte-Reassign, flow, gutachter/team, termin/sv-lead-ablehn)
-- auf claims.sv_id migriert sind (Phase D) — sonst wuerde faelle.sv_id fuer diese Writer stale.
...
DECLARE
  v_sv_id uuid;
BEGIN
  -- CMM-49 Phase F: sv_id aus claims (SSoT) fuer NOT-NULL-Constraint auf gutachten — nicht mehr faelle.
  SELECT c.sv_id INTO v_sv_id FROM public.claims c WHERE c.id = p_claim_id;
