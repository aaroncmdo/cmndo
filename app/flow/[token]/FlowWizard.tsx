...
export default function FlowWizard({ token }: { token: string }) {
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center px-5">
        <div className="text-center max-w-xs">
          ...
          <h1 className="text-2xl font-semibold text-white mb-3">Schaden gemeldet</h1>
          <p className="text-zinc-400 text-sm leading-relaxed">
            Wir haben deinen Schaden erfasst und melden uns in Kürze bei dir.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex flex-col">
      {/* Progress bar */}
      <div className="fixed top-0 inset-x-0 z-10 h-1 bg-zinc-800">
        <div
      ...
      {/* Step counter */}
      <div className="fixed top-3.5 right-4 z-10 text-xs text-zinc-500 tabular-nums">
        ...
        <div className="space-y-3 pt-4">
          ...
          {step === TOTAL_STEPS ? (
            <button
              ...
              disabled={submitting}
              className="w-full py-4 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-base disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98] transition-all"
            >
...
function SelectButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-5 py-4 rounded-xl border text-sm font-medium transition-all active:scale-[0.98] ${
        selected
      ...
      <div className="grid grid-cols-2 gap-2">
        {BEREICH_OPTIONS.map((opt) => (
          <button
            ...
            className={`text-left px-4 py-4 rounded-xl border text-sm font-medium transition-all active:scale-[0.97] ${
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full px-5 py-4 rounded-xl border border-zinc-800 bg-zinc-900 text-white text-base focus:outline-none focus:border-blue-500 transition-colors [color-scheme:dark]"
      />
    </div>
  )
}
    ...
    <div>
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-zinc-600 mt-1">Kamera oder Galerie</p>
        </div>
        <input
          ref={fileInputRef}
          type="file"
          ...
          {fotos.map((f, i) => (
            <div key={i} className="relative group aspect-square">
              ...
              <button
                ...
                className="absolute top-1 right-1 w-5 h-5 rounded-full bg-zinc-900/90 text-zinc-200 flex items-center justify-center text-xs leading-none opacity-0 group-hover:opacity-100 transition-opacity"
              >
                ×
              </button>
            </div>
      ...
      <div className="space-y-2">
        {BEWEISE_OPTIONS.map((opt) => (
          <button
            key={opt.value}
            onClick={() => toggle(opt.value)}
            className={`w-full flex items-center gap-3 text-left px-5 py-4 rounded-xl border text-sm font-medium transition-all active:scale-[0.98] ${
              value.includes(opt.value)
                ? 'border-blue-500 bg-blue-500/10 text-white'
                : 'border-zinc-800 bg-zinc-900 text-zinc-300 hover:border-zinc-600'
            }`}
...
function Step7({
  ...
  const inputCls =
    'w-full px-4 py-3.5 rounded-xl border border-zinc-800 bg-zinc-900 text-white placeholder-zinc-600 text-sm focus:outline-none focus:border-blue-500 transition-colors'
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-0.5 px-4 py-3 rounded-lg bg-zinc-900">
