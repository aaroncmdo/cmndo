...
Ablauf IM UNVERSCHULDETEN FALL (für den Kasko-/Selbstverschuldens-Zweig gilt stattdessen claimondo_finde_werkstatt): 1) Standort erfragen (PLZ oder Stadt). 2) claimondo_finde_gutachter_termine(plz ODER ort) → buchbare Gutachter + freie Slots zeigen. 3) Nutzer wählt Gutachter + Slot; Name + WhatsApp-Nummer + Schadenart + Hergang erfragen (für claimondo_melde_schaden wird eine 5-stellige PLZ benötigt — falls bisher nur eine Stadt bekannt ist, jetzt die genaue PLZ erfragen). 4) Einwilligung einholen (Datenverarbeitung + WhatsApp-Kontakt + KI-Dienst/USA), dann claimondo_melde_schaden(...) → Lead + Terminreservierung + persönlicher FlowLink per WhatsApp.
