...
**5.2 · `aktuelle_phase`-Alias (DE-3)** — ✅ **ENTSCHIEDEN (Aaron 2026-05-28): (B) Hard-Cut** auf `main_phase`/`sub_phase`. Begründung: ein Übergangs-Alias lohnt nur, wenn er **wert-transparent** ist (alte Reader laufen unverändert weiter) — hier nicht: der Wert wechselt das Vokabular (10-Code `3_gutachten_qc` → 9-Substate `gutachten`), die Parser (Kanzlei-Kanban-Ziffer-Gruppierung, `pflichtdokumente`-Cron) brechen so oder so. Der Alias brächte **0** Migrations-Sicherheit, nur eine fehlbenannte Spalte + Aufräum-Schuld → weg damit. **Sequenz (inkrementell + sicher):** 6b Views ADD `main_phase`+`sub_phase` (`aktuelle_phase` bleibt vorerst stehen, nichts bricht mid-flight) → 6a Reader **einzeln** auf `main/sub` migrieren → 6c `aktuelle_phase` + `claims.phase` + Trigger-Geflecht droppen, sobald kein Reader mehr referenziert.
