...
- **GATE A — exakte Reader-Feldmenge (mit CMM-49):** Welche `faelle`-flachen Felder lesen die ~47 entity-gated EMBED-Reader genau? §7 friert **`gegner_name` + `gegner_versicherung_name`** definitiv ein. **Teil-validiert 08.06. (grep `src/`):** `fin_vin` + `lackfarbe_code` werden von `StammdatenDetail`/`StammdatenAccordion` exakt unter diesen Namen gelesen → die geplanten View-Spalten (`veh.fin AS fin_vin`, `veh.farbcode AS lackfarbe_code`) matchen 1:1. `gegner_name`/`gegner_versicherung_name` sind §7-frozen (flach, additiv). `lackfarbe`-Klartext (`farbe_klartext`) ist von den Stammdaten-Readern **nicht** gefragt (sie nutzen `lackfarbe_code`) → optional, nur aufnehmen wenn ein Reader `farbe_klartext` braucht. **Offen bleibt nur die VOLLSTÄNDIGKEIT der 47-Reader-Menge — CMM-49s Klassifizierer (`scripts/cmm49-classify-faelle-reads.mjs`) ist die autoritative Quelle; vor dem Compose mit `fb34de27` final abgleichen** (sonst kein value-preserving Repoint).
