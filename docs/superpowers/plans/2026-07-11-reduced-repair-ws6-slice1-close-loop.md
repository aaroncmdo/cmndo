...
## Scope-Abgrenzung (Folge-Pläne)
- **Slice 1b — SV-Rechnung-Download (Normal-Claim):** SV-Upload-Pfad für eine kunde-sichtbare Honorar-/Gutachten-Rechnung (`dokument_typ` + `sichtbar_fuer` inkl. kunde) + Surfacing in der `BelegePaketCard` (Normal-Zweig). Heute voller Gap (kein Artefakt, kein Upload, admin-only Sichtbarkeit) → eigener kleiner Plan.
- **Slice 2 — Ops-Sichtbarkeit + Cron:** `deriveRepairPhase` → `v_claim_workstate` (470d55c9) + `repair-reminders`-Cron im VPS-crontab aktivieren (VPS-Lane). Eigener Plan nach Slice 1 + Koordination.
