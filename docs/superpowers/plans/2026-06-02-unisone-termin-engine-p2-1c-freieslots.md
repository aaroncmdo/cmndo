...
**Review-Befunde adressiert (02.06.):** Spec-Compliance = SPEC-COMPLIANT (genau 6 Files, 0 DDL, kein Repoint, Defaults == Originale). Adversarial (opus): **1 CRITICAL gefixt** — SV-Puffer `15`→`TERMIN_PUFFER_MIN` (=60!) + `TERMIN_DAUER_MIN`/`KB_BERATUNG_DURATION_MIN` statt Magic-Numbers (4× Under-Block-Parity-Bug; Live-Verify hatte zuvor schon den 0-Slots-Default-Bug gefangen). Minor gefixt: Phantom-Assignee-Guard (`maybeSingle`→keine Slots wenn Row fehlt statt Default), stale Verify-Kommentar. **TZ-Frage entschieden:** `setHours`-Server-lokal == beide Originale, kein neuer Bug; `toBerlinWallClock` wäre ein eigenes Cross-Cutting-Ticket, NICHT hier. **Phase-3-Parity-Flags (inherited/deferred, nicht P2.1c-blockierend):** (1) v_belegung-aktiv-Set (`reserviert/bestaetigt/verlegt/verlegung_pending`) vs Original-Blocklist (alles ausser `storniert/abgelehnt/abgesagt`) → `verschoben`/`gegenvorschlag`/`sv_gesucht` blocken in der Engine NICHT mehr (bewusste SSoT-Semantik beim Repoint bestätigen; lebt in v_belegung/P2.1a). (2) KB-Vorlauf (2h) ist window-driven → KB-Repoint muss `vonIso = now + 2h` übergeben.
