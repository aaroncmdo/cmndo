...
4. **Dependents von `fall_id` lösen (PFLICHT vor Column-Drop — sonst `ERROR 2BP01: cannot drop column ... other objects depend on it`):**
   ...
   - **⚠️ LIVE ≠ REPLAY (KEY-Finding Pilot — wichtigste Lektion):** `apply_migration`-Erfolg auf live ist KEIN Beweis für Replay-Safety. Viele Policies wurden auf live via **untracked DDL** auf claim_id repointet — im Fresh-Replay (db reset / `Supabase Preview`) hängen sie aber noch an `fall_id`. Der live-Dependency-Check (apply_migration-Fehler) zeigt sie dann NICHT, und der `DROP COLUMN` scheitert NUR im Replay (`2BP01`). → **Repointe JEDE Policy der Tabelle idempotent auf claim_id** (auch die, die live schon claim_id zeigt; Definition == live übernehmen). **Das echte Gate ist der `Supabase Preview`-Check (Fresh-Replay), nicht der live-Apply — vor Merge MUSS Supabase Preview grün sein.** (Im Pilot bei `ki_gespraeche_staff_fall_scoped` übersehen → Preview rot → Migration-File nachgebessert.)
...
7. **`Supabase Preview`-Gate + Edit-Skip-Falle (KEY-Finding Pilot):** `Supabase Preview` (Fresh-Replay) ist das Replay-Gate und MUSS vor Merge grün sein. **ABER:** ein **In-Place-Edit** einer bereits applizierten Migration (gleiche Version) wird von Preview **übersprungen** (Versions-Dedup) → der Fix wird NICHT frisch re-validiert (Preview zeigt „skipped" / weiter den alten Fail). Konsequenz für Replay-Fixes: **(a)** den Fix als **neue** Migrationsversion machen (triggert Preview) — wenn die Replay-Reihenfolge es zulässt; sonst **(b)** deduktiv beweisen via PG-Dependency-Errors (PG listet ALLE Dependents: Live-Drop-Erfolg fixiert das Live-Set, Preview-Error das Replay-Delta); **(c)** der nächste Batch-PR re-validiert den Replay ohnehin (Preview replayt dann alles inkl. des editierten Files). Plugin-`create_branch` hilft NICHT — es nutzt die Migrationen der integrierten Branch, nicht den PR-Edit; und DDL-Replay-Tests via `execute_sql` sind per Regel 2 verboten (READ-only).
