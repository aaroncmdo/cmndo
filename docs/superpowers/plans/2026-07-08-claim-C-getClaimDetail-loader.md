...
D migrates the presentations onto `getClaimDetail`. Do this as **separate plans/PRs per role**, subagent-driven, each with a prod smoke — never one big-bang.

**Status:** ✅ **D-kunde** (commit f5fc599e5) + ✅ **D-sv** (ae15c8742 facade support + 8b3321dcd page) migrated — the two cleanly-migratable role pages. **Kanzlei has no fallakte page** (LexDrive) → moot. Only D-admin/kb remains.

- **🔴 D-admin/kb** (the hard one — its OWN focused session, NOT a marathon-end push): `src/app/faelle/[id]/page.tsx` (**1130 lines**, kb+admin share it via `userRolle` branches). **Core-loader reality (live-verified):** it uses **`getFallById` → `Record<string,unknown>` from `v_faelle_mit_aktuellem_termin`** — a **FLAT shape, NOT `ClaimFull`**. So the facade's staff-core (`getClaimForRole` → `ClaimFull`) does **not** match the monolith's `fall` shape (consumed by ~40 child prop-contracts). Migration = EITHER (a) add a **third staff-core variant** to the facade that uses `getFallById` (flat, matches the monolith 1:1 → behavior-preserving, the kunde/sv pattern), OR (b) remap ~40 props from flat→`ClaimFull`. **(a) is strongly preferred** — it mirrors how kunde/sv were done (reuse the page's existing flat loader as the role-core). ⚠ The SV "safe-partial" trick (migrate only pflicht/lifecycle, keep the core) does **not** apply here: `getClaimDetail` always loads the core, so a partial would add a redundant core read. It's all-or-nothing → do the full (a) swap. The ~20 staff-only side-concerns **stay** and hang off C: QC/Vollständigkeits-Card, VS-Korrespondenz, Kanzlei-Paket + QR, Gutachten-OCR, Belege-Review, Ad-hoc-Anforderungen, KB-Phase-Audit, Regulierung-Card, Werkstatt-Vermittlung-Panel, Claim-AI-Panel (⚠ owned by 876a45e8/ad4c0df0 — coordinate; do not restructure), other-open-faelle banner. Preserve every defensive `.catch()` (AAR-650) + the admin-after-gate pattern. Add Next-Best-Action + inline-edit (`canEditField`) + Stepper on top of C. If it needs an alert, reuse the shared `<KundeAlert>` (kunde-detail-rebuild session) — no second build. FallPhasenPanel: extend via variant flag, never fork.
