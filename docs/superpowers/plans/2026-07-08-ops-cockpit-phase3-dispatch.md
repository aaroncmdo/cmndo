...
>
> **✅ RESOLUTION (found in `src/app/dispatch/leads/[id]/page.tsx:112-114`):** NOT a coordination/shared-grant problem — it's the ESTABLISHED codebase pattern. `flow_links` is default-deny for `authenticated` *by design*; the dispatch lead-detail page reads it via **`createAdminClient()` after `requirePortalAccess`** (route-gate). So the lead-cockpit loader does the SAME: (a) `v_lead_workstate` = `security_invoker=true`, granted **service_role only** (`revoke ... from anon, authenticated` → not audit-flagged, not app-exposed); (b) `getLeadWorkItems` verifies the caller is staff (dispatch/admin) — **role-guard REQUIRED** — then reads the view via the admin client. **⚠️ Security: adminClient WITHOUT the role-guard = IDOR** (see memory [[audit-route-role-gating]]) → the guard is mandatory, and this is why it wants a fresh, careful build (not a marathon-session hack). No shared-table grant, no cross-session coordination. Revised view DDL: same select, but `revoke all ... from anon, authenticated; grant select ... to service_role;`.
