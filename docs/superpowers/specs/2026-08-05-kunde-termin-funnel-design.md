...
### 4.9 Tranche W — Werkstatt-Termin-Sicht (Aaron-Nachtrag 05.08.; Diagnose gehaertet 08.08., Session 04f5e9c3)

Zwei Werkstatt-seitige Befunde derselben Klasse (tote Legacy-Achse / fehlende Row). Root-Causes am 08.08. file:line- und live-DB-hart verifiziert:

- **W1 (Haftpflicht): Werkstatt sieht den Gutachtertermin nicht — Root-Cause ist der VIEW-Join, nicht fehlende UI.** `v_werkstatt_auftrag` traegt die Besichtigungs-Spalten (`besichtigung_start/_ort/_status`, `gutachter_firmenname`) und `WerkstattAuftragDetail.tsx:392-416` rendert sie bereits. Aber der LATERAL-Join der View selektiert `gutachter_termine` via **`t.claim_id = c.id`** (reine Legacy-Achse) — Engine-/T1-Termine sind bezug-nativ (`bezug_typ='fall'`, `bezug_id=claim`, `claim_id` NULL) → Join matcht ~nie → Spalten NULL → UI-Block (`if (!besichtigung_start) return null`) verschwindet. Zusaetzlich: kein Status-Filter (`ORDER BY start_zeit DESC LIMIT 1` — ein stornierter Termin verdraengt den aktiven) und das UI-Status-Label ist binaer (`'bestaetigt' ? 'bestätigt' : 'reserviert'` — dispatch_pending/sv_gesucht wuerden falsch als „reserviert" laufen). **Soll:** View-LATERAL bezug-aware (SQL-Aequivalent von `bezugOrExpr('fall')`: `(bezug_typ IN ('fall','claim') AND bezug_id=c.id) OR (bezug_typ IS NULL AND (claim_id=c.id OR fall_id=c.id))`) + Status-Ausschluss (`storniert/abgesagt/abgelehnt/verlegt`) via `CREATE OR REPLACE VIEW` (identische Spaltenliste → Grants bleiben; reloptions null, Gate liegt im WHERE `is_werkstatt_for_claim`). UI: Status-Label um „wird bestätigt" (dispatch_pending/sv_gesucht) erweitern. ⚠ Gate-Luecke notiert: das termin-bezug-Ratchet scannt TS, NICht View-DDL — View-Migrationen brauchen Review-Aufmerksamkeit.
- **W2 (Kasko/Selbstzahler): Werkstatt kann keinen Termin setzen — Root-Cause `WerkstattAuftragDetail.tsx:80-81`:** `if (!terminId) return null` — ohne `reparatur_termine`-Row keine Sektion (der Server-Upsert `upsertWerkstattVorschlag` koennte via admin-Client laengst inserten, die UI laesst ihn nicht). Rows fehlen auf ALLEN post-Convert-Bindungspfaden (live-DB 08.08.): Quelle `embed` 7/7 MIT Row (Convert legt sie an, `convert-lead-to-claim.ts:905`, Bedingung `lead.reparatur_werkstatt_id`) · Quelle `kunde` (Akte-Finder via `assignReparaturWerkstatt`) **2/2 OHNE** · Quelle **`qr_referral` 6/6 OHNE** (DB-Trigger `set_reparatur_werkstatt_from_qr` promoted beim claims-INSERT aus `werkstatt_id`; der Convert-Insert prueft nur `lead.reparatur_werkstatt_id` → sieht die Promotion nicht; 0 UPDATE-Promotionen im Bestand → TS-Fix am Convert reicht, kein neuer DB-Trigger). **Soll — EIN Anlage-Muster fuer ALLE Pfade:** (a) idempotenter Helper `ensureReparaturTerminAngefragt` (admin-Client; legt `status='angefragt', wunschtermin=null` an, wenn keine offene Row [`angefragt/werkstatt_vorschlag/anruf_erbeten/bestaetigt`] existiert — Invariante „max. 1 offene Row/Claim" gilt empirisch bereits, 0 Verstoesse); (b) Aufruf in `assignReparaturWerkstatt` (non-fatal, auf `effectiveClaimId` — deckt Akte/Dispatch/KB/Lead-Sync) und im Convert auf den EFFEKTIVEN Claim-Zustand nach Insert (deckt qr_referral) statt nur `lead.reparatur_werkstatt_id`; (c) Backfill-DML-Migration fuer Bestands-Claims (gebunden + keine Row + `operative_status` weder `abgeschlossen/storniert/abgelehnt` noch — NULL zaehlt als offen; Stand 08.08.: 8 Claims, u.a. Aarons Repros CLM-2026-01838 + CLM-2026-02378); (d) **Kunde-Kompatibilitaet (Pflicht, da „angefragt ohne Wunschtermin" bisher ungelebt ist):** `schlageReparaturTerminVorPortal` blockt heute bei aktiver Row („Es liegt bereits ein Terminwunsch vor") — bei aktiver `angefragt`-Row MIT `wunschtermin IS NULL` stattdessen Wunschtermin per Service-Client nachtragen (Owner-Check am Rand; Kunde-RLS-UPDATE erlaubt nur werkstatt_vorschlag-Uebergaenge, daher kein user-scoped Update moeglich); (e) Werkstatt-UI-Fallback: Sektion rendert ohne Row den „Terminvorschlag offen"-Zustand mit Vorschlag-Form (`schlageWerkstattTerminVor` braucht nur `claim_id`) — Defense-in-Depth gegen kuenftige Pfad-Luecken. Die Werkstatt-UI kennt den Null-Wunsch-Zustand schon (`:100-106`, Badge „Terminvorschlag offen") — mit Row funktioniert sie unveraendert.

RLS-Lage (verifiziert): SELECT/UPDATE decken die Werkstatt via `is_werkstatt_for_claim`; INSERT nur Staff/Kunde-Party (der Werkstatt-Vorschlag-INSERT laeuft bereits admin-seitig — kein RLS-Umbau noetig). Journey-Deltas: j01 (Werkstatt sieht Begutachtungstermin read-only inkl. „wird bestätigt") + j04 (Werkstatt-Bindung ⇒ offene `reparatur_termine`-Row; Werkstatt kann proaktiv vorschlagen).

Tranche W ist NICHT Teil des T1-T3-PRs (#5012); eigener PR (Branch `kitta/kunde-termin-funnel-w`).
