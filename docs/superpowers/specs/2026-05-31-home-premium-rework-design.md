...
8. **Finale Hero-Entscheidung + Art-Direction (Aaron):** **Beides (C)** — Hero B (Paar+App) bleibt der Conversion-Hook, **„Ein Team hinter Ihrem Fall" wird prägnante Section #7** (Team-Foto, direkt unter der Trust-Strip). Übergreifende **Art-Direction für ALLE Sektionen:** bild-geführt, **viele GROSSE Full-Bleed-Foto-Bänder im NATIVEN Ratio** — die Batch ist gemischt: **mehrheitlich 19:9 ultrawide-cinematic** (sofa · sv-andreas-app · werkstatt-app · kundin-app · berater = schmale Kino-Bänder mit viel Luft) + **16:9** (hero-paar · team-band · sv-vor-ort = mehr Höhe für Scrim+Text, ideal für Hero). **Native Ratio nutzen (`object-cover` Full-Bleed), NICHT zwangs-croppen.** Dazu **offen & luftig** (großzügiger Whitespace, viel Raum zwischen Gruppen), **aufregend/dynamisch** — weg von dichten Card-Grids, hin zu cinematischen Foto-Sektionen mit Scrim + Text. Das ist die kfzgutachter-LP-Bar. Konsequenz für §3: mehr Foto-Band-Sektionen, weniger Karten; Assets in großen Breiten (1100–1920w).
