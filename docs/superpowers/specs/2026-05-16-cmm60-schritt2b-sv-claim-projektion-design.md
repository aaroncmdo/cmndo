...
**Erfolgskriterium:** `v_claim_sv` existiert, liefert einem authentifizierten SV genau seine Claims mit genau den 61 Whitelist-Spalten; ein Smoke unter echtem SV-Auth-Kontext zeigt: eigene Claims sichtbar, fremde nicht, Kanzlei-/Regulierungs-Spalten nicht vorhanden.
...
- Explizite **Spalten-Whitelist** im `SELECT` (61 Spalten) — das ist das Column-Scoping.
...
| Rechtlich / PII-minimal (5) | `verjaehrt_am` (Verjährung = rechtlich), `vs_ablehnungs_grund` (Regulierungs-Seite), `lead_id` + `geschaedigter_user_id` (interne FKs/User-IDs ohne SV-Nutzen), `kunde_email` (SV-Kunden-Kontakt strikt über `claim_parties`, Aaron-Entscheidung 2026-05-16) | Lifecycle-fremd bzw. PII-Minimierung |
...
AS
  SELECT
    ...
    -- … alle 61 Whitelist-Spalten explizit …
...
1. **Struktur:** `v_claim_sv` existiert, `security_invoker=true`, hat exakt 61 Spalten, keine der 21 ausgeschlossenen.
