...
- [ ] **4b** — `feldmodus/AktuellerStopCard.tsx` C9-Reimplementierung (Card-bg transparent statt `bg-white shadow-sm` → Glass scheint durch; expandierbare Pflichtdoku-Liste + Briefing-Accordion statt Plain-Text; Amber-Warnungen-Box; das tote `manualMode`/Collapse-Toggle verdrahten) + Glass-Card-Overlay über der Map in `FeldmodusClient` (`GlassPanel` mit `SvFallakteView`/`FokusHeader`, `navy/30 backdrop-blur`). **Eigene fokussierte Session** — Reimplementierung des C9-Styling/Layout auf der Auto-Arrive-Architektur, braucht `FeldmodusClient`/`FeldmodusStop`/`glass/*`-Studium. Der aktuelle Auto-Arrive-Card ist funktional in der Zwischenzeit.
