...
- [x] **4c** — `feldmodus/FeldmodusClient.tsx` C9-Layout-Restructure: der `8f088031`-Merge hatte ein Pre-#600-Layout eingeschleppt (staging's map+`lg:w-[380px]`-sidebar-Split), das mit den C9-Floating-Cards + dem Mobile-Bottom-Sheet doppelt rendert (`SvFallakteView` 2×). Zurück auf das kohärente C9-Layout: Map = Full-Screen-Background (`absolute inset-0 z-0`), `FokusHeader`-Pill top-left (`hidden md:block` — war importiert-aber-ungenutzt), arrived → `SvFallakteView` als zentrierter Modal-Popover (`hidden md:block fixed inset-0 bg-claimondo-navy/30 backdrop-blur-sm` + `GlassPanel variant=prominent`, `w-[min(720px,92vw)]`, mit `onBackToRoute={handleBackToRoute}`) statt 380px-Sidebar-Slot (= Aaron-Smoke MAP3), sonst → `AktuellerStopCard` mid-left + Kommende-Stops bottom-left als Floating-Glass-Cards; Mobile-Bottom-Sheet unverändert. Auto-Arrive-Logik (Geofence, `onArrived`, `markArrived`, der `useId()`-Channel aus #844) + alle anderen Overlays (NaviHud/TbtBanner/GPS-Banner/WakeLock/FokusChatPanel) unangetastet. `npm run build` grün. **(PR folgt)**
