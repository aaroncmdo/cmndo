...
| C2 | Ein Intake: createCase | A4, B1 | 🟡 **Code läuft, Tranchen a+b done (11.08.)** — Zelle war bis 12.08. auf „Plan done, Code gated" stehengeblieben, obwohl seit 04.08. Code gemergt ist (Status-Nachzug nach §8.1 versäumt). Ist-Stand verifiziert (alle PRs `MERGED`): **C2a** `createCase`-Modul + Wizard-A-1-Adapter (#4992, 04.08.) · **C2b-1** Pflichtdok-Slots im Konversions-Kern (#5126, 11.08.) · **C2b-Rest** Embed-Werkstatt-Finder B-1 + Aircall-Inbound D-4b (#5137, 11.08.). **Bewusst NICHT done — Zahl nachgemessen 19.08.: es sind SECHS, nicht ~13.** `check:intake-funnel` meldet exakt die Gate-Baseline: `admin/faelle/anlegen` · `dispatch/kalender/spontan` · `dispatch/leads` · `gutachter-vermittlung` · `flotte/schaden-fortsetzung` · `makler/erstelle-anfrage`. Die älteren Nennungen (flow, schaden/[token], matelso, public-rueckruf) sind erledigt bzw. bewusst allowlistet (NFC-Gegner-Flow).
⚠ **Die Restliste ist eine Prüfliste, keine Schuldenliste.** Nachgemessen (90 Tage, `leads` ⟕ `flow_links`): **kein einziger echter Endkunde ohne FlowLink** — 14 Treffer, davon 5 `schaden-karte` (by design), 8 Test-/Smoke-Daten, 1 SV-Interessent über `/gutachter/willkommen`. Zwei der sechs Wege (`flotte/schaden-fortsetzung`, `gutachter-vermittlung`) rufen `ensureCanonicalFlowLinkForLead` **selbst** → strukturell abgedeckt. Zwei erzeugten in 90 Tagen **0 Leads** (`admin-direkt`, `dispatch_spontan`) → ungenutzt, nicht harmlos. Wer die Tranche aufnimmt, beginnt bei **`dispatch/leads/actions.ts`** (0 FlowLink-Bezüge, `source_channel` ist dort ein Parameter — über den Kanal nicht auffindbar). Details: `REFERENCE-prod-messungen-claims-was-man-vorher-wissen-muss` | #4992 · #5126 · #5137 |
| C3 | Notification-Outbox | A3, C1 | 🟡 **Code läuft, Outbox live mit echten Consumern (11.08.)** — Zelle war bis 12.08. auf „Plan done, Code gated" (Nachzug versäumt). Verifiziert (alle `MERGED`): **C3a** enqueue + Worker (#5011, 08.08.) · toter `dispatch-fall-actions.ts`-Pfad entfernt (#5090) · **echter Consumer** in `process-event.ts` (#5109, 09.08.) · **Cron-Kunden-Sends** durable (#5131, 11.08.) · **Fall-Lifecycle-Kunden-Sends** durable (#5139, 11.08.). **Für §9-#6 (13.08.):** Die Abdeckungs-Erhebung (#5250) zeigte, dass die Outbox real ~nichts abdeckte (1 Zeile in 14 Tagen gegen 523 `notification_deliveries`) — Ursache war strukturell, nicht Rückstand. **Aaron-Entscheid „mach es durable" ist umgesetzt (#5252):** `sendFallCommunication` enqueued jetzt selbst, jeder Aufrufer bekommt Dedup + Retry + Dead-Letter. 🔴 **Prod-Nachweis am 19.08. erhoben — er fällt NEGATIV aus, der Haken bleibt zu.** `notifications_outbox` enthält **2 Zeilen insgesamt** (nicht: in 14 Tagen — insgesamt), gegen **645 `notification_deliveries` in 14 Tagen**. Beide Outbox-Zeilen stammen vom **19.08. 04:16/04:17 UTC**, also aus den nächtlichen Smoke-Läufen (Template `fall_eroeffnet`) — **im echten Betrieb: null**. Die Erhebung #5250 sah 1 gegen 523; #5252 hat die Quote also nicht bewegt.
⚠ **Das heißt NICHT, dass die Zustellung ungesichert ist** — die Präzisierung ist wichtig, sonst wirkt der Befund dramatischer als er ist. Es gibt **zwei** Wege:
1. **Ereignis-basiert** (`notification_events` → fan-out → `notification_deliveries`, `api/notifications/process`): **bereits durable** — Retry mit `next_retry_at`, Dead-Letter (`status='failed'` + `next_retry_at IS NULL`), Fehler in `error_message`. Prod-Stand: **813 `sent`, 74 `skipped` mit Grund, 0 `failed`** — die Null ist belastbar, weil der Code `failed` sehr wohl setzt (drei Stellen, inkl. `catch`).
2. **Direkt** (`sendCommunication` & Co., ~40 Files): **nicht durable** — dafür ist die Outbox gedacht, und genau die ist ungenutzt.

Die Rest-Tranche betrifft also Weg 2. #5252 hat die obere Ebene (`sendFallCommunication`) darauf umgestellt; die Masse läuft weiter direkt.
Rest-Tranche: die untere Ebene `sendCommunication` (~40 Files) — Einzelprüfung je Call-Site, kein Sammel-Refactoring. ⚠ Ein Regel-4-Smoke über die obere Ebene würde die Outbox zwar füllen und „grün" aussehen — er belegt dann aber nur den Pfad, den die Smokes ohnehin nehmen. Der Nachweis muss an **echtem** Verkehr hängen. C3c (SA-Moment 6–7 WA, Marker `COORDINATION-sa-moment-wa-konsolidierung-handoff`) ist der meistzitierte Rest | #5011 · #5090 · #5109 · #5131 · #5139 |
| C4 | Eine Akte (rollen-parametrisierter Kern) | B1 | ✅ **done (14.08.)** — §9-#7 gehakt: **alle SECHS Rollen-Detailsichten am Kern**. Zuletzt migriert: **Makler** (#5277, tabs) und **Flotte** (#5283, stack). Die dritte Position der alten Rest-Liste („Kunde als zweite parallele Sicht") existierte nicht — `/kunde/faelle/[id]` rendert bereits `<KundeClaimView>`, und `FallDetailSections` ist ein Baustein INNERHALB davon (nur von `DoksTermineZone` importiert). Alt-Implementierungen waren mit #5184 erledigt. ⚠ Dauerkriterium: eine neue Rolle mit eigener Fall-Detailsicht öffnet den Haken wieder | #4940 · #4977 · #5184 · #5277 · #5283 |
...
- [ ] Ausgehende Kommunikation läuft über die Outbox mit Dedup; 0 offene P1-Lücken aus A3 — *Abdeckungs-Erhebung 13.08.: **Der C3a-Handoff ist abgearbeitet, der Punkt bleibt trotzdem offen — und zwar aus einem strukturellen Grund, nicht wegen Rückstands.** (a) **Infra + Dedup funktionieren**: `ON CONFLICT (dedup_key) DO NOTHING`; die eine prod-Zeile ging beim ersten Versuch raus (`status='sent'`, `versuche=0`). (b) **14 Consumer** verdrahtet (6 Crons + `kanzlei-paket`, `termine`, `nachbesichtigung`, `sv-zuweisung`, `lexdrive/process-event`, `konditional-tasks`, `convert-lead-to-fall`, `kunde-5min-notification`) — praktisch die komplette Liste aus [[handoff-fundament-c3a-outbox-retarget]], inkl. **begründeter** Auslassungen: Crons sind semi-durable (nächster Lauf retryt), `storno-actions.ts:94` dokumentiert, dass `no_show_kunde` nie feuert, `sla/kanzlei-mahnungen.ts` hat mit `n_mahnungen`/`letzte_mahnung_am` ein **eigenes** Stufen-Tracking. (c) **Aber die Masse lief strukturell vorbei:** die Communications-Schicht selbst (`send-fall.ts`, `channel-router.ts`, `channels.ts`) kannte die Outbox **nicht** (0 Treffer). Real 14 Tage: `email_log` 304 · `notification_deliveries` 523 · `nachrichten` 97 — gegenüber **1** Outbox-Zeile. (d) ✅ Die im C3a-Marker vermerkte Lücke `kanzlei_wartet_auf_kunde_wa` **existiert nicht mehr** (steht in `communications/registry.ts:419`).
→ ✅ **Aaron-Entscheid 13.08. („mach es durable") UMGESETZT — #5252:** `sendFallCommunication` schreibt jetzt selbst in die Outbox; jeder Aufrufer bekommt Dedup + Retry + Dead-Letter-Task ohne eigenes Zutun. Drei Fallen dabei entschieden: **Endlosschleife** (der Worker ruft `sendFallCommunicationDirekt`, statischer Guard-Test mit verifiziertem Rot-Fall) · **Dedup-Granularität** (Key trägt eine Tagesscheibe — ohne sie hätte jeder Trigger pro Fall nur EINMAL gefeuert und jeder Reminder wäre still verschwunden) · **FK** (`notifications_outbox.claim_id` → `claims(id)`; die Claim-ID wird vor dem Enqueue aufgelöst, da Aufrufer auch `faelle.id` übergeben dürfen). Ausfall der Outbox → Direktversand, kein neuer Single-Point-of-Failure.
⚠ **Der Haken bleibt offen bis zum Prod-Nachweis** (deploy-gated, [[handoff-regel4-smokes-durable-und-datumsfelder]]): Die Outbox-Zeilenzahl muss nach dem Deploy **steigen** — bleibt sie bei ~0, laufen alle Sends in den Direkt-Fallback und der Umbau griffe nicht.
📋 **Rest-Tranche, bewusst nicht mitgenommen:** die untere Ebene `sendCommunication` (~40 Files, davon viele fall-bezogen). Das ist **kein** Sammel-Refactoring: Wer bereits aufgelöste Empfängerdaten übergibt, passt nicht ins `(fallId, template, extra)`-Schema und darf nicht gehoben werden (Grenze aus [[handoff-fundament-c3a-outbox-retarget]]); Crons sind ohnehin semi-durable. Jede Call-Site braucht Einzelprüfung.*
- [x] Ein Akte-Kern, alle Rollen-Detailseiten migriert, Alt-Implementierungen gelöscht — *✅ **erfüllt 14.08.**: **alle SECHS Rollen-Detailsichten hängen am Kern** — Staff `FallakteShell` (tabs) · SV `FallDetailClient` (stack) · Kunde `KundeClaimView` (columns) · Werkstatt `WerkstattAuftragDetail` (columns) · **Makler `MaklerAkteDetail` (tabs, #5277)** · **Flotte `FlottenClaimDetailView` (stack, #5283)**. Verifiziert über `git grep "from '@/components/fall-akte/FallAkte'"` — es gibt keinen weiteren Consumer und keine kernlose Claim-Detailsicht. Die Kanzlei bleibt bewusst außen vor (kein C4-Ziel, keine Fall-Detail-Route).
  ⚠ **Die Rest-Liste vom 12.08. war zu dritt genannt, real waren es zwei:** (i) Makler und (ii) Flotte stimmten und sind migriert; **(iii) „Kunde `faelle/[id]` als zweite, parallele Fallsicht" gibt es nicht.** Nachgemessen 14.08.: `/kunde/faelle/[id]/page.tsx` rendert `<KundeClaimView>` (also den Kern), und `FallDetailSections` wird **ausschließlich** von `claim-view/DoksTermineZone` importiert — es ist ein Baustein INNERHALB der migrierten Sicht, keine Sicht daneben. Keine zweite Route rendert es. Wer die alte Formulierung liest, migriert etwas, das es nicht gibt.
  📌 **Dauerkriterium (Aaron-Entscheid 12.08.):** „alle Rollen-Detailseiten" schließt KÜNFTIGE Rollen ein. Der Haken gilt für den heutigen Stand — eine neue Rolle mit eigener Fall-Detailsicht öffnet ihn wieder. Prüfbefehl steht oben.
  Alt-Implementierungen: bereits 12.08. abgeschlossen (6 tote Files entfernt, knip-Baseline 54→48, #5184); die 2 verbliebenen sind kein Akte-Alt-Code (`fall/TerminVorschlagModal` wird von `shared/TerminCard` genutzt, `lib/finance/fall-finanzen` von einem Guard-Test per `readFileSync` gelesen).*
