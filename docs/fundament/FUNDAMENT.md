...
- [ ] Ausgehende Kommunikation läuft über die Outbox mit Dedup; 0 offene P1-Lücken aus A3 — *Abdeckungs-Erhebung 13.08.: **Der C3a-Handoff ist abgearbeitet, der Punkt bleibt trotzdem offen — und zwar aus einem strukturellen Grund, nicht wegen Rückstands.** (a) **Infra + Dedup funktionieren**: `ON CONFLICT (dedup_key) DO NOTHING`; die eine prod-Zeile ging beim ersten Versuch raus (`status='sent'`, `versuche=0`). (b) **14 Consumer** verdrahtet (6 Crons + `kanzlei-paket`, `termine`, `nachbesichtigung`, `sv-zuweisung`, `lexdrive/process-event`, `konditional-tasks`, `convert-lead-to-fall`, `kunde-5min-notification`) — praktisch die komplette Liste aus [[handoff-fundament-c3a-outbox-retarget]], inkl. **begründeter** Auslassungen: Crons sind semi-durable (nächster Lauf retryt), `storno-actions.ts:94` dokumentiert, dass `no_show_kunde` nie feuert, `sla/kanzlei-mahnungen.ts` hat mit `n_mahnungen`/`letzte_mahnung_am` ein **eigenes** Stufen-Tracking. (c) **Aber die Masse läuft strukturell vorbei:** `sendFallCommunication`/`sendCommunication` wird in **53 Files** aufgerufen, und die Communications-Schicht selbst (`send-fall.ts`, `channel-router.ts`, `channels.ts`) kennt die Outbox **nicht** (0 Treffer). Real 14 Tage: `email_log` 304 · `notification_deliveries` 523 · `nachrichten` 97 — gegenüber **1** Outbox-Zeile. (d) ✅ Die im C3a-Marker vermerkte Lücke `kanzlei_wartet_auf_kunde_wa` **existiert nicht mehr** (steht in `communications/registry.ts:419`). → **Entscheidung nötig (Aaron):** Der Weg zu diesem Haken ist NICHT, 53 Call-Sites einzeln zu heben, sondern die Frage, ob `sendFallCommunication` **selbst** durable wird (ein Eingriff statt 53, alle Sends automatisch mit Dedup + Dead-Letter). Solange das nicht entschieden ist, bleibt der Punkt offen — nicht wegen fehlender Arbeit, sondern wegen fehlender Architektur-Entscheidung.*
