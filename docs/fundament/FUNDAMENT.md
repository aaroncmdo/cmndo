...
- [ ] Ein Akte-Kern, alle Rollen-Detailseiten migriert, Alt-Implementierungen gelöscht — *DoD-Erhebung 12.08. (B0): **Kern + Migration erfüllt**, `src/components/fall-akte/FallAkte.tsx` + 3 Layouts, rollen-parametrisiert (`FallakteRolle`→`viewerRole`), **5 Consumer über 4 Rollen-Oberflächen** (Staff/SV/Kunde/Werkstatt) aus #4940+#4977. **Beide ursprünglichen Rest-Punkte sind inzwischen geklärt:** (1) **Alt-Implementierungen gelöscht** — `knip` meldete 8 Akte-/Fall-Kandidaten; **6 einzeln als tot verifiziert und entfernt** (`FallActivityFeed`, `fall/DokumentenListe`, `lib/fall/{communication-timeline,sla-config,urls}`, `gutachter/fall/[id]/cardentity-actions`), knip-Baseline **54→48** (PR #5184, tsc + vitest grün). **2 bleiben bewusst stehen und sind KEIN Akte-Alt-Code:** `fall/TerminVorschlagModal` (wird von `shared/TerminCard` importiert — offizielle Composite-Komponente) und `lib/finance/fall-finanzen` (wird von `leadkosten-source.test.ts` per `readFileSync` als Guard-Liste gelesen; Löschen bräche den Test). (2) **Kanzlei ist kein C4-Ziel** — ⚠ **Korrektur einer Handoff-Aussage:** #4977 heißt „SV+Werkstatt+**Staff**", **nicht** „…+Kanzlei…", und `c4-eine-akte-plan.md` erwähnt die Kanzlei **überhaupt nicht** (0 Treffer). Die Kanzlei hat auch keine Fall-Detail-Route (nur `kanzlei/mandate` als Liste). Der C4-Scope sind die **5 Rollen** aus #4977 (Kunde/SV/Werkstatt/Staff-Varianten), alle am Kern. **✅ Scope-Entscheid Aaron (12.08., DECISIONS 2026-08-12): „alle Rollen-Detailseiten" schließt künftige Rollen EIN** — §9-#7 ist damit ein **Dauerkriterium**, kein Momentaufnahme-Haken (deckt sich mit Verfassung §1-4 „Neue Rolle = Konfiguration, nicht neues Portal"). **Erhebung dazu: 3 Claim-Detailsichten hängen NICHT am Kern** → der Punkt bleibt offen mit konkreter Rest-Liste: **(i) Makler** `(shell)/akten/[id]` (eigene Komponente `components/makler/akte-detail/MaklerAkteDetail`) · **(ii) Flotte** `(shell)/fahrzeug/[id]/schaden/[claimId]` (eigene Implementierung, liest `from('claims')`) · **(iii) Kunde** `faelle/[id]` (`FallDetailSections` + `shared/fall-header`/`fall-kontakte` — **parallel** zur bereits migrierten `KundeClaimView`-Variante, also zwei Kunde-Fallsichten nebeneinander). ⚠ `flotte/*` + Kunde-Zonen gehören fremden Lanes ([[coordination-an-b0e963b6-claim-detail-fm-vs-kunde-split]]) → Migration ist zuweisbare Tranche, kein Beifang.*
