...
| C1 | Ein Status-Writer: transitionClaim + Event-Log | A2, B1 | 🟢 **Code komplett gemergt (11.08.), Regel-4 offen** — C1a-Funnel (#4935) + **C1-Finish (#5114): Ratchet-Baseline 2→0**, die 2 letzten Direkt-Writer (`kanzlei-wunsch/actions.ts`, `termine/close-nur-gutachter-termin.ts`) auf `transitionFallStatus` gehoben. Die beiden Nicht-Matrix-Terminals (`an_externe_kanzlei_uebergeben`/`termin_durchgefuehrt`) sind jetzt `BROADLY_REACHABLE_TERMINALS` (aus jedem AKTIVEN Zustand erreichbar, Muster `storniert` — die Direkt-Writes hatten keine Source-Guard) + Engine setzt `abgeschlossen_am`; 4 tote Smoke-Reset-Actions (0 Consumer) gelöscht statt allowlistet. **Voll-Achsen-Verifikation 11.08.:** TS-Achse `check:operative-status-writes` = 0 Verletzer (2781 Files) · **DB-Achse = 0** (Prod-READ: kein Trigger, keine Function schreibt `operative_status`; `cron_verjaehrungs_warner` liest nur) · `reparatur-cursor.ts` = Engine-Wrapper · `endzustand-actions` (Allowlist) schreibt eigenes `phase_transitions`+Emit. **Rest:** Regel-4-Prod-Smoke (deploy-gated) + §9-Event-Log-Bullet. | #4845 · #4935 · **#5114** |
...
- [ ] Event-Log wird bei jedem Übergang geschrieben; seit C1 keine neue `ist_*`/`hat_*`-Interaktions-Spalte — *Stand 11.08.: Engine (`phase_transitions`) + `endzustand-actions` (eigenes) + `reparatur-cursor` (via Engine) schreiben; die 2 mit #5114 gefunnelten Terminals ab jetzt ebenfalls. Offen: (a) Prod-Nachweis = Regel-4-Smoke, (b) dokumentierte Feinlücke `lexdrive manual_status_override` schreibt kein `phase_transitions` (Scope-Zaun §0.2 → Fund notiert, nicht als Beifang gefixt; siehe DECISIONS 2026-08-11).*
