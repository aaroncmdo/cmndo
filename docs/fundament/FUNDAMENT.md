...
| B3 | Journey-Suite J2–J10 in CI (Tranchen) | B1, B2 | 🟢 **in Arbeit** (8c6de199, 03.08.) — **T1** (#4948): J9 `verrechnung`+`staffel`. **T2**: J8 `2fa-enroll` (Seed+Step). Klassifikation + Skip-Begründungen in `journey-smokes.md` (§9: grün ODER begründeter Skip). Offen: **J5** (Fixture-Claim 39734007 zustandsgedriftet → eigener Seed nötig), **J10** (`.env`→`process.env`-Fix). J2/J3/J6/J7 + J9-`lifecycle` = begründeter opt-in-Skip | #4948 + T2-PR |
