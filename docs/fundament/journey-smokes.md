...
**Stand — 04.08. (Aaron-Direktive „keine Skips"):** §9-Punkt-2 gilt erst als **erfüllt, wenn alle 10 Journeys wirklich CI-grün** laufen — ein begründeter Skip ist nicht länger das Ziel (die frühere „§9 via Skips erfüllt"-Deklaration war voreilig). **Grün in CI = J1/J4/J9/J8/J5** (J5 neu 04.08.: deterministischer kasko-Seed, aal1-Login; lokal grün Seed 5/5 + Smoke 1 passed). **Im Bau (Skip → CI-Step):** J10 (`db()`→`process.env` + Szenario-1-Isolation + Seed-cleaning), J3 (dedizierter SA/Vollmacht-Seed+Spec), J6 (Kanzlei-Übergabe-Seed+Spec), J7 (Skeleton → echte Storno/DSGVO-Logik über Wegwerf-Konto), J2 (Multi-Kanal-Meldeweg-Seed mit Send-Isolation), J9-`lifecycle` (Release-Cron Test-Row-Isolation — ggf. Produkt-Change, mit Aaron abzustimmen).
