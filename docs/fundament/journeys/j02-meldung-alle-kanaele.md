...
2b. **Melder hat den FlowLink nicht** (nie angekommen, verloren, Gerät gewechselt) → öffnet `/login`, gibt Telefonnummer oder E-Mail an → System: existiert ein Lead/Claim zu diesem Kontakt, entsteht sein Konto (Stufe 1: nur wenn der Lead eine E-Mail trägt), er bekommt SMS-Code bzw. Anmelde-Link; unbekannte Kontakte sehen denselben neutralen Satz (Enumeration-Schutz). Nach dem Login: Fall zugeordnet (`claims.geschaedigter_user_id`, auch per Telefon-Suffix), offener Lead als Karte „Jetzt fortsetzen“ → frischer FlowLink → weiter bei Schritt 2. **Festnetz ohne E-Mail:** kein digitaler Weg — die Dispatch-Aufgabe bei `kanal='none'` (#5986) ist der vorgesehene Pfad. Soll-Blatt `memory/abnahmen/2026-09-19-kunde-kommt-ohne-link-ins-konto.md`, Plan `docs/superpowers/plans/2026-09-19-kunde-login-ohne-link.md`.
