...
1. **Nachwirkungen NICHT zentral garantiert** — sie liegen verteilt auf `createLead` / `convertLeadToClaim`-Kern / `convertLeadToFall`-Wrapper / `create-pflicht.ts` / `ensureCanonicalFlowLink`; **kein** Meldeweg bekommt automatisch alle sechs. → das ist der C2-Auftrag. **Teilfortschritt:** der Kern legt seit C2b-1 (11.08.) die **Pflichtdok-Slots** selbst an (s. #2); FlowLink bleibt Sache von `createCase`/`ensureCanonicalFlowLinkForLead`.
