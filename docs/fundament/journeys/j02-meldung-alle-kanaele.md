...
1. **Nachwirkungen NICHT zentral garantiert** — sie liegen verteilt auf `createLead` / `convertLeadToClaim`-Kern / `convertLeadToFall`-Wrapper / `create-pflicht.ts` / `ensureCanonicalFlowLink`; **kein** Meldeweg bekommt automatisch alle sechs (`convert-lead-to-claim.ts` legt weder Pflichtdok noch FlowLink an). → das ist der C2-Auftrag.
2. **Gegner-Flow/Schadenkarte konvertiert zu Claim OHNE Pflichtdok-Slots** (`schaden/[token]/actions.ts:179` ruft den Kern direkt, nicht den Wrapper) — der Claim hat keine Upload-Platzhalter.
