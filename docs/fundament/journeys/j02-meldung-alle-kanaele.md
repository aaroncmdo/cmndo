...
2b. **Melder hat den FlowLink nicht** (nie angekommen, verloren, Gerät gewechselt) → öffnet `/login`, gibt Telefonnummer oder E-Mail an → System: existiert ein Lead/Claim zu diesem Kontakt, entsteht sein Konto — seit Stufe 2 (20.09., Migration `20260920155109`) auch **nur mit Telefonnummer**, ohne E-Mail; er bekommt SMS-Code (6 Stellen, 60 s) bzw. Anmelde-Link; unbekannte Kontakte sehen denselben neutralen Satz (Enumeration-Schutz). Nach dem Login: Fall zugeordnet (`claims.geschaedigter_user_id`, auch per Telefon-Suffix), offener Lead als Karte „Jetzt fortsetzen“ → frischer FlowLink → weiter bei Schritt 2. **Festnetz ohne E-Mail:** kein digitaler Weg — die Dispatch-Aufgabe bei `kanal='none'` (#5986) ist der vorgesehene Pfad. Soll-Blätter `memory/abnahmen/2026-09-19-kunde-kommt-ohne-link-ins-konto.md` (Stufe 1) und `2026-09-20-kunde-konto-nur-telefon-stufe-2.md` (Stufe 2), Pläne `docs/superpowers/plans/2026-09-19-kunde-login-ohne-link.md` / `2026-09-20-kunde-konto-nur-telefon.md`. Besitz eines Vorgangs im Portal: E-Mail **oder** Telefon-Suffix (`lib/kunde/besitz.ts`), NULL ist nie ein Treffer.
