...
2. **Kunde wählt Gutachter-Termin** (Slot eines passenden SV) → System reserviert den Slot **soft** (`sv-termin`); **Notif:** SV `termin.sv_bestaetigt` (web_push/in_app), Kunde WA-Bestätigung. **Angebotene Slots sind immer in der Zukunft** (T5, 08.08.): der Wunschtermin-Picker bietet für heute nur Uhrzeiten nach der aktuellen Stunde an und lässt heute ganz weg, sobald kein Slot mehr übrig ist — kein `start_zeit < created_at` mehr (Spec 2026-08-05 §1 Randbefund). **Wunschtermin-Zweig (kein passender/echter SV, z. B. Dead-Pin):** der gewählte Slot wird als Terminwunsch festgehalten (Termin-Status `dispatch_pending`/`sv_gesucht`), der Kunde sieht ihn durchgängig als „Wunschtermin · wird bestätigt" (Akte-Terminsektion + Termine-Hub), und der Wunsch landet in der Dispatch-Terminwunsch-Queue mit SLA — nie ownerlos (Spec 2026-08-05 §2/§4.3/§4.4).
