...
## Ausführungs-Stand (2026-05-24)

**PR1 — Ownership-Konsolidierung: FERTIG (clean boundary), Build-grün, gepusht, kein PR (smoke-gated).**
- Commits: `38159d30` (faelle/[id]: beratung/besichtigungsort/kalender) · `35ddcbea` (onboarding ×5 + nachbesichtigung) · `a1b8f3c8` (2 IDOR-Guards: setzeVorschadenAbrechnung + completeOnboarding).
- 11 inline `fall.kunde_id!==user.id` → `assertKundeOwnsFall` (claim_parties-SSoT). kunde `from('faelle')` 39→28.

**PR2 — Route-Key-Switch faelle.id→claim_id: FOUNDATION gelegt (`d89527ea`, Build-grün, behavior-preserving).**
- `assertKundeOwnsClaim(claimId)` (claim-nativ, claim_parties-primär, liefert fallId-Transitions-Bridge).
- `getKundeFallDetailRecord` = **accept-both** (claim_id zuerst, faelle.id-Fallback) → Alt-URLs unverändert, claim_id-URLs neu. faelle bleibt Basis-Row (kennzeichen/fahrzeug_*/zahlungsweg/bankdaten noch faelle-nativ bis SP-E/CMM-65).

**PR2 — verbleibende Schritte (BEHAVIOR-FLIP, smoke-gated):**
1. `getKundeFaelle` Rückgabe `id = claim.id` (statt faelle.id) → Linker (KundeNav/FallKarte/layout singleFallId+fallOptionsForChat) emittieren claim_id. **Flip-Punkt.**
2. Redirect-Canonicalize: `/kunde/faelle/<faelleId>` → 308 → `<claimId>` (map via faelle solange es lebt) für Alt-Bookmarks/SEO.
3. Deferred Reader-Rebase auf claim_parties/claims: `onboarding/getFreieSlotsFuerKunde` (claims-Embed), `nachbesichtigung/page` (gibt faelle.id zurück), `termine/[id]` (fahrzeug+termin via gutachter_termine.claim_id), `chat`/`kb-chat`/`layout`×5/`completeOnboarding:540` (keyed-Reads, created_at-order = CMM-65-gekoppelt).
4. Sub-Routen `nachbesichtigung/[fall_id]`/`termine/[id]`-Param-Semantik.

**Smoke vor jedem Flip-PR (Pflicht, lokal — Branch nicht auf staging):** `next dev` im Worktree + Test-Kunde → `/kunde`, `/kunde/faelle/[id]` (claim_id-URL + alte faelle.id-URL→redirect), `/kunde/onboarding`, `/kunde/nachbesichtigung`, `/kunde/termine` → Owner sieht / Fremder 403/notFound. Screenshot je Route.
