...
- **Spalten-Partition (gegen Flat-Modell-Recreation, Aaron 04.06. — der eigentliche Sinn der Seam):** **entity-bound** Spalten (Person/Fahrzeug/Gegner/Versicherer) → Entitäten (Entity ownt via Writer + View), **nie flach nach `claims`**; **claim-native** (status/workflow/refs/dates) → `claims` (CMM-49). **P3-Writer von entity-Daten → Entity-Writer** (`ensurePersonForData` etc.; `person_id`-Wiring ist live in `convert-lead-to-claim`/`flow`/`airdrop`), **nicht** flach-`claims`. Diese Writer müssen **semantisch dedupen** (find-or-create per kanonischem Key, NICHT exakt-String) → **eine SoT pro realem Ding über alle Claims, alle Referenzen routen darauf (FK), kein Dupe**. **Stand live verifiziert 04.06.:** ✅ Personen (`ensurePersonForData`+`match_person_candidates`: user_id / name+gebdat / verified-contact) · ✅ Fahrzeuge MIT FIN (`upsert_vehicle_by_fin`). **⚠️ Resolver-LÜCKEN = Entity-Writer-Wiring-TODO (noch KEIN find-or-create live):** Versicherer (95-Zeilen-Registry existiert, aber kein Match), Werkstätten, Mietwagenunternehmen, **Firmen-als-Partei** (`personen.ist_gewerbe`/`firma`), **Fahrzeuge OHNE FIN** (Kennzeichen-Resolver) — und es gibt **keine `canonical/normalized`-Spalten** für semantischen Match. **Bis ein Typ-Resolver steht: diese Daten NICHT flach-per-Claim schreiben** (sonst semantische Dupes) — via Entity-Writer (sobald da) oder halten. Backstop: Identity-Engine (`verified_contacts`/`match_person_candidates`) + read-only Dublettenliste (#2382).
