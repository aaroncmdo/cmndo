...
const OUT = join(__dirname, 'smoke-claims-stufe-0.5b-screenshots')
...
async function visit(page, path, label, watchFor = []) {
  ...
  page.on('console', (msg) => {
    if (msg.type() === 'error') {
      const t = msg.text()
      if (/column "(firma_ustid)" does not exist|v_claim_full.*does not exist|permission denied/i.test(t)) errors.push(t.slice(0, 200))
    }
  })
  try {
    ...
    r.columnErr = /column "(firma_ustid)" does not exist|v_claim_full.*does not exist/i.test(html)
...
writeFileSync(join(OUT, '..', 'smoke-claims-stufe-0.5b-result.json'), JSON.stringify(results, null, 2))
