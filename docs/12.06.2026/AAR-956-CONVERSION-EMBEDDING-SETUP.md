...
| **`gf_anfrage_submit`** | **Reservierung abgeschickt** (Kontakt → Danke) | `{ schadenart:'haftpflicht', value:100, currency:'EUR', lead_id?, phone? }` | **Haupt-Conversion (Lead)** |
| **`gf_rueckruf`** | **Rückruf/Beratung gebucht** (auf der Danke-Seite) | `{ schadenart:'schadensberatung', value:25, currency:'EUR', lead_id? }` | **Conversion (Beratungs-Lead)** |
...
### `lead_id` & `phone` — warum sie wichtig sind
...
* **`phone`** = die Kundennummer in **E.164** (`+49…`, via `toE164`). Für **Enhanced Conversions**:
  GTM hasht sie clientseitig (SHA-256) und schickt sie an Google → bessere Zuordnung trotz
  Cookie-Verlust. Nur bei `gf_anfrage_submit` vorhanden.
...
`event`, `cl_event_source`, `value`, `currency`, `lead_id`, `phone`, `schadenart`, `gutachter`, `context`
...
* Je Conversion-Action ein Ads-Conversion-Tag:
  * **Reservierung** → Trigger `gf_anfrage_submit`, Conversion-Value `{{value}}`, Currency `{{currency}}`,
    Transaction-ID `{{lead_id}}`, **Enhanced Conversions an** → User-Data-Variable mit `{{phone}}` (schon E.164).
  * **Rückruf** → Trigger `gf_rueckruf`, Value `{{value}}`, Transaction-ID `{{lead_id}}`.
...
* `lead_id`-Transaction-ID + Enhanced-Conversions (`phone`) puffern Cookie-/Cross-Domain-Verlust ab.

---

## 5 · GTM-Container in die Embed-Seite bringen (kleiner Code-Schritt — OFFEN)

Aktuell pusht der Code nur in `window.dataLayer` — **es lädt noch kein GTM-Container im iframe**
(`src/app/embed/gutachter-finder/` hat keinen GTM-Snippet). Damit die Pushes irgendwo ankommen,
muss der Container geladen werden. Sauberer, env-gegateter Weg (lädt nur wenn konfiguriert):

```tsx
// src/app/embed/gutachter-finder/layout.tsx  (oder page.tsx), via next/script
import Script from 'next/script'
const GTM = process.env.NEXT_PUBLIC_GF_GTM_ID   // z.B. "GTM-GFXXXXX"
// …
{GTM && (
...
* ENV `NEXT_PUBLIC_GF_GTM_ID` auf der **app.claimondo.de**-Deployment (VPS Portal :3000) setzen.
* Sag Bescheid — diesen Loader (env-gegated) baue ich dir gern in einem kleinen Folge-PR, sobald
  du die Container-ID hast. (Bewusst nicht jetzt reingebaut, damit ohne ID nichts Halbes lädt.)
