...
test.describe('Finder-Sprungpanel — Desktop', () => {
  ...
  test('beide Knoepfe sind da und bleiben zurueckhaltend', async ({ page }) => {
    ...
    // „nicht so praesent" (Aaron): klein und am Rand, nicht bildschirmfuellend.
    ...
    // ⚠ RECHTS, nicht links: links liegt der Lead-Wizard („4 kurze Fragen").
    // Der erste Prod-Screenshot zeigte das Panel komplett ueber ihm — ein
    // Panel vor dem Conversion-Element zieht den Kunden genauso weg.
  ...
  test('das offene Panel verdeckt den Lead-Wizard NICHT', async ({ page }) => {
    // Der Grund fuer die Position rechts. Gefunden auf dem ersten
    // Prod-Screenshot: das Panel lag vollstaendig ueber „4 kurze Fragen".
    await page.goto(FINDER)
    await oeffneStaedte(page)
    const panel = await page.getByRole('dialog', { name: 'Städte' }).boundingBox()
    const wizard = await page.locator('iframe').first().boundingBox()
    expect(panel).not.toBeNull()
    expect(wizard).not.toBeNull()
    // Der Wizard sitzt in der linken Haelfte des Embeds — das Panel muss
    // rechts davon beginnen.
    expect(panel!.x).toBeGreaterThan((wizard?.width ?? 1440) * 0.45)
  })
