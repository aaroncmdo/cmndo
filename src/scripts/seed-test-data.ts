...
async function ensureUser(u: TestUser): Promise<string> {
  ...
  if (error && error.message.includes('already been registered')) {
    ...
    if (authRows?.[0]?.id) {
      ...
      await db.from('profiles').upsert({ id: authRows[0].id, email: u.email, vorname: u.vorname, nachname: u.nachname, telefon: TEST_PHONE, rolle: u.rolle, twofa_aktiviert: false, twofa_email_aktiviert: false, force_password_change: false }, { onConflict: 'id' })
    ...
    while (page <= 20) {
      ...
      if (found) {
        u.userId = found.id
        await db.from('profiles').upsert({ id: found.id, email: u.email, vorname: u.vorname, nachname: u.nachname, telefon: TEST_PHONE, rolle: u.rolle, twofa_aktiviert: false, twofa_email_aktiviert: false, force_password_change: false }, { onConflict: 'id' })
  ...
  await db.from('profiles').upsert({
    ...
    // AAR-562: force_password_change aus — sonst Redirect nach /passwort-aendern vor Cookie-Set.
...
async function main() {
  ...
  // 4. Faelle/Claims — CMM-49 faelle-DROP: das (claimlose) Fall-Seeding ist entfernt
  // (faelle ist gedroppt; claimlose Faelle passen ohnehin nicht ins claim-first-Modell).
  // Dieser Seeder legt Test-User + SV + Leads an; Faelle/Claims entstehen ueber den
  // kanonischen Converter (convertLeadToClaim) bzw. die Admin-"Fall anlegen"-Action.
