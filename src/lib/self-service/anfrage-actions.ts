...
// AAR-956 Phase C (Decoupling-Schritt 1): route-neutrale Self-Service-Booking-Actions.
// War src/app/anfrage/[token]/actions.ts; 1:1 hierher gezogen, damit die GETEILTEN
// Onboarding-Bausteine (WizardClient/TerminField, auch in /kunde- + /gutachter-
// Onboarding) nicht mehr an der /anfrage-Route hängen. Die /anfrage-Route re-exportet
// von hier (Shim) und verschwindet beim Phase-C-Drop — diese Lib bleibt.
//
// AAR-940 Phase 2: Token-validierte Self-Service-Strecke (gfa.self_service_token).
// Anon — kein Login. Promotion Anfrage->Lead beim Klick (service_role, anon schreibt
// nie in leads). Muster: /kunde-termin/[token] (createAdminClient, Token+Expiry-Gate).
