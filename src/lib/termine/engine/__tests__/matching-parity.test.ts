...
describe('findeBestePerson Parität (Sub-A.1) — Sticky-Bonus', () => {
  it('Sticky +1000 hebt einen sonst schwächeren Kandidaten über den Best-Score', () => {
    const schwach = bewerteSvKandidat({ paket: 'standard', kontingentGenutzt: 5, ablehnungen30d: 0, etaVomBueroMin: 30, distanzKm: 20 })
    const stark = bewerteSvKandidat({ paket: 'premium', kontingentGenutzt: 0, ablehnungen30d: 0, etaVomBueroMin: 10, distanzKm: 5 })
