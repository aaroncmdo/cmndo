...
// Erkennt Slot-Reservierungen in gutachter_finder_anfragen, die seit mehr als
// 24 Stunden gehalten werden (slot-ttl-cleanup-Cron hat sie nicht freigegeben).
// Read-only auf gutachter_finder_anfragen.reservierter_slot_von.
...
export const slotsStaleReservationsCheck: HealthCheck = {
  ...
  async run(ctx): Promise<CheckResult> {
    ...
    return {
      status,
      metric: nStale,
      detail: `${nStale} Slot-Reservierungen >24h gehalten (älteste ${alterAnzeige}) — slot-ttl-cleanup prüfen`,
    }
  },
}
