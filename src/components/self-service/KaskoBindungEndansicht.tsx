...
export function KaskoBindungEndansicht({
  ...
  return (
    <div className={kompakt ? 'flex flex-col gap-3' : 'max-w-md w-full flex flex-col gap-4'} data-testid="kasko-bindung-endansicht">
      <div>
        <h1 className={titel}>{du ? 'Ihr Kasko-Tarif enthält eine Werkstattbindung' : 'Ihr Kasko-Tarif enthält eine Werkstattbindung'}</h1>
      ...
      <Card p={4} radius="lg" accentColor="warning">
        <p className="text-body-sm font-semibold text-claimondo-navy">{du ? 'Was das für Sie bedeutet' : 'Was das für Sie bedeutet'}</p>
        <p className="mt-1 text-body-sm text-claimondo-navy/80">{info.sanktionText}</p>
        {info.nachlassText && (
          <p className="mt-2 text-caption text-claimondo-navy/60">{du ? 'Dafür erhalten Sie den Beitragsnachlass: ' : 'Dafür erhalten Sie den Beitragsnachlass: '}{info.nachlassText}.</p>
        )}
      </Card>
      ...
      <Card p={4} radius="lg">
        ...
        <ol className="mt-2 list-decimal space-y-1 pl-5 text-body-sm text-claimondo-navy/80">
          <li>
            {du ? 'Melden Sie den Schaden Ihrer Versicherung' : 'Melden Sie den Schaden Ihrer Versicherung'}
      ...
      {onRueckruf && rueckruf !== 'fertig' && (
        <Button variant="ghost" fullWidth onClick={() => void anfordern()} loading={rueckruf === 'sendet'}>
          <span data-testid="kasko-bindung-rueckruf">{du ? 'Rückruf anfordern – wir beraten Sie zum weiteren Vorgehen' : 'Rückruf anfordern – wir beraten Sie zum weiteren Vorgehen'}</span>
        </Button>
      )}
      {rueckruf === 'fertig' && <p className="text-body-sm text-success-strong">{du ? 'Danke – wir rufen Sie zurück.' : 'Danke – wir rufen Sie zurück.'}</p>}
      {onKorrigieren && (
      ...
      <p className="text-caption text-claimondo-navy/50">
        {du ? 'Maßgeblich sind Ihr Versicherungsschein und Ihre AKB.' : 'Maßgeblich sind Ihr Versicherungsschein und Ihre AKB.'} Diese Einschätzung beruht auf dem Tarifnamen (Stand {info.stand}
