...
export function KaskoBindungEndansicht({
  ...
  return (
    <div className={kompakt ? 'flex flex-col gap-3' : 'max-w-md w-full flex flex-col gap-4'} data-testid="kasko-bindung-endansicht">
      <div>
        <h1 className={titel}>{du ? 'Dein Kasko-Tarif enthält eine Werkstattbindung' : 'Ihr Kasko-Tarif enthält eine Werkstattbindung'}</h1>
        ...
        <p className="mt-2 text-body-sm text-claimondo-navy/80">
          {du
            ? 'Deine Versicherung benennt die Reparaturwerkstatt. Eine Werkstatt-Vermittlung durch uns ist in diesem Fall nicht möglich – damit dir keine Kürzung entsteht.'
      ...
      <Card p={4} radius="lg" accentColor="warning">
        <p className="text-body-sm font-semibold text-claimondo-navy">{du ? 'Was das für dich bedeutet' : 'Was das für Sie bedeutet'}</p>
        <p className="mt-1 text-body-sm text-claimondo-navy/80">{info.sanktionText}</p>
        {info.nachlassText && (
          <p className="mt-2 text-caption text-claimondo-navy/60">{du ? 'Dafür erhältst du den Beitragsnachlass: ' : 'Dafür erhalten Sie den Beitragsnachlass: '}{info.nachlassText}.</p>
        )}
      </Card>
      ...
      <Card p={4} radius="lg">
        ...
        <ol className="mt-2 list-decimal space-y-1 pl-5 text-body-sm text-claimondo-navy/80">
          <li>
            {du ? 'Melde den Schaden deiner Versicherung' : 'Melden Sie den Schaden Ihrer Versicherung'}
          ...
          <li>{du ? 'Lass dir die Partnerwerkstatt benennen' : 'Lassen Sie sich die Partnerwerkstatt benennen'}{info.partnernetz ? ` (${info.partnernetz})` : ''}.</li>
          <li>{du ? 'Ausnahmen, bei denen du frei wählen darfst: ' : 'Ausnahmen, bei denen Sie frei wählen dürfen: '}{info.ausnahmenText}.</li>
        </ol>
      </Card>

      {onRueckruf && rueckruf !== 'fertig' && (
        <Button variant="ghost" fullWidth onClick={() => void anfordern()} loading={rueckruf === 'sendet'}>
          <span data-testid="kasko-bindung-rueckruf">{du ? 'Rückruf anfordern – wir beraten dich zum weiteren Vorgehen' : 'Rückruf anfordern – wir beraten Sie zum weiteren Vorgehen'}</span>
        </Button>
      )}
      {rueckruf === 'fertig' && <p className="text-body-sm text-success-strong">{du ? 'Danke – wir rufen dich zurück.' : 'Danke – wir rufen Sie zurück.'}</p>}
      {onKorrigieren && (
      ...
      <p className="text-caption text-claimondo-navy/50">
        {du ? 'Maßgeblich sind dein Versicherungsschein und deine AKB.' : 'Maßgeblich sind Ihr Versicherungsschein und Ihre AKB.'} Diese Einschätzung beruht auf dem Tarifnamen (Stand {info.stand}
