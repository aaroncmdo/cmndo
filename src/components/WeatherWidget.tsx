...
export default function WeatherWidget({ lat, lng }: { lat: number | null; lng: number | null }) {
  ...
  if (error) {
    return (
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-4 max-w-[300px]">
        <p className="text-zinc-500 text-xs">Wetter konnte nicht geladen werden.</p>
      </div>
    )
  }
  if (!weather) {
    return (
      <div className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-5 max-w-[300px] animate-pulse">
  ...
  return (
    <div className="bg-zinc-900/60 backdrop-blur-md border border-zinc-700/50 rounded-2xl p-5 max-w-[300px] w-full">
      {/* City */}
      <p className="text-zinc-400 text-xs font-medium uppercase tracking-wide mb-2">{weather.city}</p>

      {/* Temp + Icon row */}
      <div className="flex items-center gap-3 mb-2">
        <span className="text-[32px] font-bold text-white leading-none tabular-nums">{weather.temp}°</span>
        <img
      ...
      {/* Description */}
      <p className="text-zinc-300 text-sm capitalize mb-3">{weather.description}</p>
        ...
        <div className="flex items-center gap-3 pt-3 border-t border-zinc-800">
          {weather.forecast.map((f, i) => (
            <div key={i} className="flex flex-col items-center gap-0.5 flex-1">
              <span className="text-zinc-500 text-[10px]">{formatForecastTime(f.time)}</span>
              <img
                ...
                className="w-7 h-7"
              />
              <span className="text-zinc-300 text-xs font-medium tabular-nums">{f.temp}°</span>
            </div>
          ))}
        </div>
      )}
