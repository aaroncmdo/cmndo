      ...
      <div className="flex flex-col h-full w-full lg:max-w-lg lg:h-auto lg:max-h-[90vh] lg:rounded-xl lg:shadow-2xl lg:overflow-hidden bg-white">
        ...
        <div className="px-4 py-2 bg-gray-50 border-b border-gray-200 shrink-0">
          <div className="flex items-center justify-between text-xs mb-1">
            ...
            <span className="text-blue-600 font-semibold">{Math.round((erledigt / total) * 100)}%</span>
          </div>
          <div className="h-1.5 bg-gray-200 rounded-full">
            <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${(erledigt / total) * 100}%` }} />
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {FOTO_PERSPEKTIVEN.map(p => (
                <label key={p} className={`flex items-center justify-center gap-2 py-3 rounded-lg border-2 border-dashed cursor-pointer transition-colors ${
                  fotos[p] ? 'border-green-300 bg-green-50 text-green-600' : 'border-gray-200 bg-gray-50 text-gray-500 hover:border-blue-300'
                }`}>
        ...
        <div className="px-4 py-3 border-t border-gray-200 shrink-0">
          <button onClick={handleComplete} disabled={completing || fotosErledigt < 4}
            className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-sm font-medium py-3 rounded-lg transition-colors">
