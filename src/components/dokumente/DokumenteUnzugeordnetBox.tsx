...
export default function DokumenteUnzugeordnetBox({
  ...
  return (
    <div className="bg-warning-soft/40 border border-warning/30 rounded-ios-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-warning/30 flex items-center justify-between">
        ...
        {docs.map((doc) => (
          <div key={doc.id} className="px-4 py-2.5 flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <FileTextIcon className="w-4 h-4 text-warning-strong shrink-0" />
              <div className="min-w-0">
                <p className="text-sm text-claimondo-navy truncate">
                  {doc.original_filename ?? 'Unbenannt'}
                </p>
                <div className="flex items-center gap-2 mt-0.5">
                  <StatusBadge colorCls="bg-white/70 text-warning-strong border border-warning/30">
