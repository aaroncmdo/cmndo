...
export default function RealtimeCountView({
  ...
  if (variant === 'dot') {
    return (
      <span
        ...
        className={`inline-block w-2 h-2 rounded-full bg-danger ${className}`}
      />
    )
  }
  ...
  return (
    <span
      aria-label={`${count} neu`}
      className={`inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full text-[10px] font-bold bg-danger text-white ${className}`}
    >
      {count > 99 ? '99+' : count}
    </span>
  )
