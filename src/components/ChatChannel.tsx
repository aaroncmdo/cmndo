          ...
          return (
            <div key={m.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-3 py-2 rounded-xl text-sm ${isOwn ? 'bg-claimondo-shield text-white rounded-br-sm' : 'bg-claimondo-bg text-claimondo-navy rounded-bl-sm'}`}>
                ...
                <p className={`text-[9px] mt-1 ${isOwn ? 'text-claimondo-light-blue' : 'text-claimondo-ondo/70'}`}>{new Date(m.created_at).toLocaleTimeString('de-DE', { timeZone: 'Europe/Berlin', hour: '2-digit', minute: '2-digit' })}</p>
              </div>
            </div>
          )
        })}
