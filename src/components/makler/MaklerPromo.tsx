  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className="bg-white rounded-2xl border border-[#e4e7ef] p-5 space-y-5">
          <div>
            <p className="text-[11px] uppercase tracking-wider text-[#4573A2] font-medium">
              Ihr Promo-Code
            </p>
            <div className="mt-2 flex items-center gap-2">
              <code className="flex-1 font-mono text-2xl md:text-3xl font-bold text-[#0D1B3E] bg-[#f8f9fb] border border-[#e4e7ef] rounded-lg px-4 py-3 tracking-wider">
                {code}
              </code>
              <button
                type="button"
                onClick={() => copy(code, 'code')}
                className="shrink-0 inline-flex items-center justify-center w-11 h-11 rounded-lg bg-[#0D1B3E] text-white hover:bg-[#1E3A5F]"
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-2">
              <input
                readOnly
                value={landingUrl}
                className="flex-1 font-mono text-sm text-[#0D1B3E] bg-[#f8f9fb] border border-[#e4e7ef] rounded-lg px-3 py-2.5 truncate"
                onFocus={(e) => e.currentTarget.select()}
              />
              <button
                type="button"
                onClick={() => copy(landingUrl, 'url')}
                className="shrink-0 inline-flex items-center gap-1.5 px-3 h-10 rounded-lg bg-white border border-[#e4e7ef] text-sm text-[#0D1B3E] hover:border-[#4573A2]"
              >
                {copied === 'url' ? (
                  <>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[11px] uppercase tracking-wider text-[#4573A2] font-medium inline-flex items-center gap-1.5">
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={downloadPng}
                  className="inline-flex items-center gap-1.5 px-3 h-8 rounded-lg bg-[#0D1B3E] text-white text-xs font-semibold hover:bg-[#1E3A5F]"
                >
                ...
                <button
                  type="button"
                  onClick={downloadSvg}
                  className="inline-flex items-center gap-1.5 px-3 h-8 rounded-lg bg-white border border-[#e4e7ef] text-xs font-semibold text-[#0D1B3E] hover:border-[#4573A2]"
                >
            ...
            <div
              className="flex items-center justify-center p-4 rounded-xl bg-[#f8f9fb] border border-[#e4e7ef]"
              dangerouslySetInnerHTML={{ __html: qrSvg }}
            />
            <p className="mt-2 text-[11px] text-[#7BA3CC] text-center">
        ...
        <section className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-[#e4e7ef] p-5">
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <a
                href={mailHref}
                className="inline-flex items-center gap-2 px-3 h-10 rounded-lg bg-[#0D1B3E] text-white text-sm font-semibold hover:bg-[#1E3A5F]"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-3 h-10 rounded-lg bg-white border border-[#e4e7ef] text-sm font-semibold text-[#0D1B3E] hover:border-[#4573A2]"
              >
                {copied === 'share' ? (
                  <>
...
function StatCard({
  ...
  const toneMap: Record<typeof tone, string> = {
    ...
    navy: 'bg-[#0D1B3E]/5 text-[#0D1B3E] border-[#0D1B3E]/10',
    green: 'bg-emerald-50 text-emerald-700 border-emerald-200',
    orange: 'bg-orange-50 text-orange-700 border-orange-200',
  }
  return (
    <div className="rounded-xl border border-[#e4e7ef] bg-[#f8f9fb] p-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[11px] uppercase tracking-wider text-[#4573A2] font-medium">
          {label}
        </span>
      </div>
      <p className="mt-1.5 text-xl font-bold text-[#0D1B3E]">{value}</p>
    </div>
  )
}
