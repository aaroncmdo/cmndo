---
publish_status: live
brand: claimondo.de
url: /haftpflicht/mietwagen
type: glossar-spoke
cluster: H3
nummer: H3.5
primary_keyword: "mietwagen nach autounfall"
secondary_keywords: ["mietwagen unfall wer zahlt", "mietwagen dauer reparatur", "mietwagen klasse unfall", "unfall-ersatz-tarif"]
insurer_phrases:
  - "Mietwagen-Tagessatz überhöht"
  - "Bei unserer Partner-Vermietung günstiger"
customer_questions:
  - "wer zahlt mietwagen nach unfall"
  - "wie lange mietwagen bei reparatur"
  - "welche mietwagen klasse erlaubt"
  - "mietwagen statt nutzungsausfall"
last_modified: 2026-05-18
last_legal_review: pending
related:
  - /haftpflicht/nutzungsausfall
  - /haftpflicht/wiederbeschaffungswert
  - /haftpflicht/reparaturkosten
  - /decoder/mietwagen-zu-hoch
---

# Mietwagen-Erstattung beim Verkehrsunfall

> **Kurz erklärt:** Wenn dein Fahrzeug nach unverschuldetem Unfall in Reparatur ist oder du auf Wiederbeschaffung wartest, hast du Anspruch auf einen Mietwagen — die Kosten trägt der gegnerische Haftpflichtversicherer (§ 249 BGB). Der **„ortsübliche Normaltarif"** ist maßgeblich, gerichtlich oft als Misch-Mittel aus Schwacke-Liste und Fraunhofer-Mietpreisspiegel berechnet. Mietwagen-Klasse maximal **eine Stufe unter** deinem eigenen Fahrzeug. Dauer entspricht der **tatsächlichen Reparatur- oder Wiederbeschaffungs-Zeit**.

## Auch bekannt als

Mietwagen-Anspruch · Ersatzfahrzeug · Leihwagen-Erstattung · UET (Unfall-Ersatz-Tarif)

---

## In einem Satz erklärt

Mietwagen sind erstattbar, wenn dein Fahrzeug unfall-bedingt nicht nutzbar ist — Tagessatz nach ortsüblichem Normaltarif, Klasse maximal eine Stufe unter deinem Fahrzeug, Dauer der tatsächlichen Reparatur.

## In drei Sätzen erklärt

Bei einem unverschuldeten Unfall mit Fahrzeug-Ausfall hast du Anspruch auf einen Mietwagen als Ersatz-Mobilität — Voraussetzungen: Reparatur-Dauer oder Wiederbeschaffungs-Zeit, kein verfügbarer Zweitwagen, angemessene Mietwagen-Klasse (BGH-Linie). Der erstattbare Tagessatz orientiert sich am **„ortsüblichen Normaltarif"**, in der gerichtlichen Praxis oft als arithmetisches Mittel aus **Schwacke-Liste** (höhere Tagessätze) und **Fraunhofer-Mietpreisspiegel** (niedrigere Tagessätze) berechnet. Maximale Mietwagen-Klasse: dein eigenes Fahrzeug — eine Klasse darunter ist Schadensminderungs-zumutbar, mehrere nicht.

## Die fünf Schlüsselzahlen

- **§ 249 BGB** als Anspruchsgrundlage
- **14 Tage** Standard-Wiederbeschaffungs-Dauer bei Totalschaden
- **Misch-Mittel** aus Schwacke + Fraunhofer als BGH-Bezug
- **1 Klasse** Reduktion zumutbar (Schadensminderung)
- **0 € Kosten** für dich bei unverschuldetem Unfall

---

## Voraussetzungen für den Mietwagen-Anspruch

Vier Bedingungen müssen erfüllt sein:

1. **Nutzungs-Ausfall des eigenen Fahrzeugs** — Reparatur oder Wiederbeschaffung
2. **Kein verfügbarer Zweitwagen** (siehe unten)
3. **Tatsächliche Anmietung** (anders als beim alternativen [Nutzungsausfall](/haftpflicht/nutzungsausfall))
4. **Mietwagen-Klasse angemessen** (max. eigene Klasse)

Bei Vorliegen aller Bedingungen ist die Mietwagen-Rechnung voll erstattbar — bei Nicht-Verfügbarkeit eines günstigeren Tarifs sogar der höhere Unfall-Ersatz-Tarif (UET).

---

## Schwacke vs. Fraunhofer — die BGH-Linie

Die zwei wichtigsten Tabellenwerke zur Bewertung „ortsüblicher Normaltarif":

| Quelle | Datenbasis | Tendenz |
|---|---|---|
| **Schwacke-Liste** (Eurotax) | Befragung von Autovermietern (Direkt-Tarife) | Höhere Tagessätze |
| **Fraunhofer-Mietpreisspiegel** | Online-Buchungs-Plattformen mit Vorbuchung | Niedrigere Tagessätze |
| **Misch-Mittel beider** | Arithmetisches Mittel | BGH-Linie in vielen OLG-Bezirken |

Der BGH lässt **beide Tabellen als zulässige Schätzungsgrundlage** zu — die gerichtliche Praxis arbeitet überwiegend mit dem Misch-Mittel. Versicherer wählen einseitig die für sie günstigere Tabelle — siehe [Decoder „Mietwagen-Tagessatz zu hoch"](/decoder/mietwagen-zu-hoch).

---

## Mietwagen-Klasse — was ist zumutbar

Die BGH-Linie zur „angemessenen Klasse":

- **Eigenes Mittelklasse-Fahrzeug** → Mittelklasse-Mietwagen erstattbar (Klasse C)
- **Eine Klasse darunter** ist zumutbar (Klasse B statt Klasse C) — Schadensminderungs-Pflicht
- **Mehrere Klassen darunter** nicht zumutbar
- **Premium-Fahrzeug (Klasse E+)** → ggf. nur Mittelklasse erstattbar (oberer Wert der zumutbaren Reduktion)

→ Praktisch: Wenn du ein Mittelklasse-Auto fährst, ist Mittelklasse-Mietwagen voll erstattbar. Versicherer-Druck zu Kleinstwagen ist nicht zulässig.

---

## Mietwagen-Dauer — was wird erstattet

Maßgeblich ist die **tatsächliche** Ausfall-Zeit:

- **Bei Reparatur:** Werkstatt-Auftrag + Reparatur-Bericht
- **Bei Totalschaden:** ca. 14 Tage Wiederbeschaffungs-Dauer (BGH-Standard)
- **Bei Sonder-Fahrzeugen oder verlängerten Lieferzeiten:** länger nachweisbar
- **Bei Ersatzteil-Wartezeit:** verlängert die Reparatur-Dauer

Versicherer-Kürzungen mit „pauschal nur 7 Tage" oder „Reparatur hätte schneller erfolgen müssen" sind oft unbegründet, wenn die tatsächliche Dauer durch Werkstatt-Belege dokumentiert ist.

---

## Mietwagen oder Nutzungsausfall — du wählst

Du kannst entweder einen Mietwagen anmieten oder pauschalierten [Nutzungsausfall](/haftpflicht/nutzungsausfall) geltend machen — beides zusammen geht nicht. Praktischer Vergleich:

| | Mietwagen | Nutzungsausfall |
|---|---|---|
| Voraussetzung | Tatsächliche Anmietung | Eigennutzungs-Vermutung |
| Höhe | Tatsächlicher Tagessatz | Sanden-Danner-Pauschale |
| Mobilität | Vorhanden | Nicht |
| Aufwand | Anmietung, Abwicklung | Keiner |
| Bei Zweitwagen | Reduziert | Reduziert |

→ Entscheidung im Einzelfall: Wenn du Mobilität brauchst, ist Mietwagen besser. Wenn du Mobilität durch Zweitwagen, ÖPNV oder Fahrrad ausgleichen kannst, ist Nutzungsausfall oft wirtschaftlich besser (Pauschale ohne tatsächliche Kosten).

---

## Was du jetzt machst — konkret

### 💡 Die wichtigste Regel
Mietwagen-Anmietung mit Verfügbarkeits-Nachweis. Wenn ein günstigerer Tarif zum Anmietzeitpunkt nicht verfügbar war, ist der tatsächlich gezahlte höhere Tarif erstattbar.

### 🛠 Vier Schritte

**1. Anmietung dokumentieren**
Mietvertrag, Tagessatz, Klasse, Dauer.

**2. Verfügbarkeit anderer Tarife dokumentieren**
Welche Anbieter waren angefragt? Welche Tarife waren verfügbar? Online-Vergleichs-Screenshots.

**3. Reparatur-Dauer dokumentieren**
Werkstatt-Auftrag, Reparatur-Bericht, Rückgabe-Datum.

**4. Versicherer-Kürzung mit Decoder antworten**
Bei Kürzung über 100 € siehe [Decoder „Mietwagen-Tagessatz zu hoch"](/decoder/mietwagen-zu-hoch).

---

## Häufige Fragen

**Wer zahlt den Mietwagen nach Unfall?**
Bei unverschuldetem Unfall die gegnerische Haftpflichtversicherung als Schadensposition nach § 249 BGB. Du gehst kein finanzielles Risiko ein.

**Wie lange darf der Mietwagen-Zeitraum sein?**
Für die tatsächliche Reparatur-Dauer (Werkstatt-Belege) oder bei Totalschaden typisch 14 Tage Wiederbeschaffungs-Dauer.

**Welche Mietwagen-Klasse darf ich nehmen?**
Maximal die Klasse deines eigenen Fahrzeugs, eine Klasse darunter ist Schadensminderungs-zumutbar.

**Was ist der Unfall-Ersatz-Tarif (UET)?**
Höherer Mietwagen-Tarif speziell für Unfall-Geschädigte (Vermieter trägt Vorfinanzierungs-Risiko). Erstattbar nur, wenn ein günstigerer Tarif zum Anmietzeitpunkt **nicht verfügbar** war.

**Was, wenn ich einen Zweitwagen habe?**
Bei tatsächlicher Verfügbarkeit des Zweitwagens kann der Mietwagen-Anspruch reduziert oder ausgeschlossen sein — abhängig von der konkreten Familien-Konstellation.

**Kann ich auch ohne Anmietung etwas bekommen?**
Ja — alternativ [Nutzungsausfall](/haftpflicht/nutzungsausfall) (Sanden-Danner-Pauschale). Du wählst eines von beidem.

---

## Verwandte Begriffe

- [Nutzungsausfall](/haftpflicht/nutzungsausfall) — Alternative ohne Mietwagen
- [Wiederbeschaffungswert](/haftpflicht/wiederbeschaffungswert) — Bezug Wiederbeschaffungs-Dauer
- [Reparaturkosten](/haftpflicht/reparaturkosten) — Reparatur-Dauer
- [Decoder „Mietwagen-Tagessatz zu hoch"](/decoder/mietwagen-zu-hoch) — Antwort auf Kürzung

---

## Wenn du nicht weiter weißt

Mietwagen-Streit ist Standard-Eskalations-Feld. Differenz zur Versicherer-Bewertung typisch 200–600 € pro Fall — leicht zurückzuholen mit Anwalt.

→ [Kostenfreie Anspruchs-Prüfung starten](https://claimondo.de/check)

---

## Quellen

- Bürgerliches Gesetzbuch §§ 249, 254 — [gesetze-im-internet.de/bgb](https://www.gesetze-im-internet.de/bgb/)
- BGH, Urteil vom 11.03.2008, VI ZR 164/07 (Schwacke als Schätzgrundlage)
- BGH, Urteil vom 24.06.2008, VI ZR 234/07 (Erforderlichkeit Mietwagen)
- Schwacke Mietpreis-Liste
- Fraunhofer-Mietpreisspiegel — fraunhofer.de
- Hentschel/König/Dauer, Straßenverkehrsrecht

---

## Schema (JSON-LD)

```json
{
  "@context": "https://schema.org",
  "@graph": [
    {
      "@type": "DefinedTerm",
      "name": "Mietwagen-Erstattung",
      "alternateName": ["Mietwagen-Anspruch", "Ersatzfahrzeug", "UET"],
      "description": "Mietwagen-Kosten sind nach § 249 BGB erstattungsfähig, wenn das eigene Fahrzeug unfall-bedingt nicht nutzbar ist. Maßgeblich ist der ortsübliche Normaltarif (Schwacke/Fraunhofer-Misch-Mittel), Mietwagen-Klasse maximal eigene Klasse, Dauer der tatsächlichen Reparatur oder Wiederbeschaffung.",
      "inDefinedTermSet": {"@type": "DefinedTermSet", "name": "Glossar Kfz-Haftpflichtschaden", "url": "https://claimondo.de/haftpflicht"}
    },
    {
      "@type": "Article",
      "headline": "Mietwagen-Erstattung beim Verkehrsunfall",
      "datePublished": "2026-05-18",
      "dateModified": "2026-05-18",
      "inLanguage": "de-DE",
      "author": {"@type": "Organization", "name": "Claimondo Redaktion"},
      "citation": [
        {"@type": "Legislation", "name": "BGB § 249"},
        {"@type": "Legislation", "name": "BGH VI ZR 164/07"}
      ]
    },
    {
      "@type": "FAQPage",
      "mainEntity": [
        {"@type": "Question", "name": "Wer zahlt den Mietwagen nach Unfall?", "acceptedAnswer": {"@type": "Answer", "text": "Bei unverschuldetem Unfall die gegnerische Haftpflichtversicherung als Schadensposition nach § 249 BGB."}},
        {"@type": "Question", "name": "Wie lange darf der Mietwagen-Zeitraum sein?", "acceptedAnswer": {"@type": "Answer", "text": "Für die tatsächliche Reparatur-Dauer oder bei Totalschaden typisch 14 Tage Wiederbeschaffungs-Dauer."}},
        {"@type": "Question", "name": "Welche Mietwagen-Klasse darf ich nehmen?", "acceptedAnswer": {"@type": "Answer", "text": "Maximal die Klasse deines eigenen Fahrzeugs, eine Klasse darunter ist Schadensminderungs-zumutbar."}}
      ]
    }
  ]
}
```

---

> **Rechtlicher Hinweis:** Dieser Beitrag ist redaktionelle Aufklärung und ersetzt keine anwaltliche Beratung im Einzelfall.

*Letzte fachliche Überprüfung: pending — Schluss-Review durch unsere Partnerkanzlei steht aus.*
