...
export async function POST(request: Request) {
  ...
  const { template, fallId, gutachterId, kanzleiEmail } = body
  ...
  try {
    switch (template) {
      ...
      case 'kanzlei_auftrag':
        ...
        break
      default:
        return NextResponse.json({ error: `Unbekanntes Template: ${template}. Erlaubt: kunde_welcome, sv_auftrag, sv_abrechnung, kanzlei_auftrag` }, { status: 400 })
    }
