      ...
      if (orParts.length > 0) {
        ...
        if (staleTermin?.claim_id) {
          ...
          if (intent === 'termin_bestaetigung_ja') {
            ...
            // Logik mit SV-/Kunde-Action). byUserId = Kunde-Profil falls vorhanden,
            // sonst null (reiner Lead ohne Account).
  ...
  if (matchedFallId && intent === 'dokument_upload' && mediaUrls.length > 0) {
    ...
    if (fallKb && gespeichert.length > 0) {
      ...
      await createNotification(
        ...
        `Kunde hat ${gespeichert.length} Dokument(e) gesendet: Fall ${fallClaim?.claim_nummer ?? matchedFallId.slice(0, 8)}`,
