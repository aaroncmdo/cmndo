      ...
      {step === 0 && (
        <div className="space-y-5">
          <div className="grid grid-cols-1 gap-3">
            ...
            <div>
              <label className="text-xs text-gray-500 mb-1.5 block">Beschreibung (optional)</label>
              <textarea
                value={beschreibung}
                onChange={e => setBeschreibung(e.target.value)}
                rows={2}
                placeholder="Worum geht es in dieser Community?"
                className="w-full bg-gray-100 border border-gray-300 rounded-xl px-3 py-2 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#4573A2] resize-y"
              />
            </div>
            <div>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-xs text-gray-500">Mindestens 1 Mitglied. Jeder bekommt eigenen Login + eigene Anzahlung.</p>
          ...
          {mitglieder.map((m, idx) => (
            <div key={m.id} className="border border-gray-200 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {mitglieder.length > 1 && (
                  <button type="button" onClick={() => removeMember(m.id)} className="text-gray-300 hover:text-red-400 p-0.5">
                  ...
                  {(['standard', 'pro', 'premium'] as const).map(p => (
                    <button key={p} type="button" onClick={() => updateMember(m.id, 'paket', p)}
                      className={`px-3 py-2 rounded-lg border text-xs transition-colors ${
                        m.paket === p ? 'border-[#4573A2] bg-[#4573A2]/5 text-[#4573A2] font-semibold' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                      }`}>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm">
            <p className="text-xs text-gray-500 uppercase mb-2">Community</p>
            ...
            <div className="mt-3 pt-3 border-t border-gray-200">
              <p className="text-xs text-gray-500 uppercase mb-2">{mitglieder.length} Mitglied(er)</p>
              {mitglieder.map((m, i) => (
                <div key={m.id} className="text-xs text-gray-700 mb-1">
