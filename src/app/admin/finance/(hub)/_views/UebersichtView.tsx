...
import { CLOSED_OPERATIVE_STATUS_PG, COMPLETED_OPERATIVE_STATUS_VALUES } from '@/lib/claims/terminal-status'
...
export default async function UebersichtView() {
  const supabase = await createClient()
  ...
  const [{ data: offeneClaimIds }, { data: abgeschlossenClaimIds }] = await Promise.all([
    ...
    // B2b: 'abgeschlossen'-Count = alle nicht-stornierten Terminals (COMPLETED). Vor der Konsolidierung
    // kollabierten alle Endzustaende auf coarse 'abgeschlossen'; nach dem endzustand-Flip tragen
    // KB-Closes feine Terminals -> COMPLETED zaehlt exakt dieselbe Fallmenge (verhaltensneutral).
    supabase.from('claims').select('id').in('operative_status', COMPLETED_OPERATIVE_STATUS_VALUES),
  ])
...
// Task-3 (FinanceHubShell): liefert die 3 Header-Pill-Werte (MRR, SVs, Mandate diesen Monat),
// die frueher im PageHeader dieser Seite selbst berechnet wurden (s. actions-Prop oben, jetzt
// entfernt). Dieselbe Query-Logik wie in UebersichtView oben — bewusst ein leichtgewichtiger
// Doppel-Query zu den 3 Aggregaten dort (Task-2-Brief: korrekt vor clever, kein Cross-Dedupe).
export async function ladeFinanceHeaderStats(): Promise<{ mrr: number; svCount: number; mandateMonat: number }> {
  const supabase = await createClient()
  ...
  const [{ data: aktiveSvs }, { data: kanzleiVollmachtenMonat }] = await Promise.all([
    // 1. Aktive SVs mit Paket -> MRR + SV-Count (identische Query wie UebersichtView).
    supabase
      .from('sachverstaendige')
      .select('paket')
      .eq('ist_aktiv', true),

    // 2. Komplett-Mandate diesen Monat (identische Filter wie UebersichtView; nur 'id'
    //    noetig, die zusaetzlichen Felder der "letzteVollmachten"-Liste braucht der Header nicht).
    supabase
      .from('v_claim_full')
      .select('id')
      .eq('service_typ', 'komplett')
  ...
  // AAR-947 / W1.3: voller Paketpreis aus der SSoT (getPaket normalisiert auch Legacy-Paket-Keys).
  const mrr = (aktiveSvs ?? []).reduce((sum, sv) => {
    return sum + (sv.paket ? getPaket(sv.paket).preis : 0)
  }, 0)
  ...
  return {
    mrr,
    svCount: (aktiveSvs ?? []).length,
    mandateMonat: (kanzleiVollmachtenMonat ?? []).length,
  }
}
