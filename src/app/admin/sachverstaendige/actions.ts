...
// AAR-132: Zentrale HERE-basierte Isochrone, kein lokales OSRM-Duplikat mehr
...
export async function onboardGutachter(data: OnboardingData) {
  ...
  if (data.standort_lat && data.standort_lng && svEntry?.id) {
    try {
      ...
      // AAR-132: Isochrone-Berechnung ist nicht kritisch — SV wird trotzdem angelegt.
      // findBestSV fällt ohne Polygon auf den Radius-Check zurück.
      console.error('[AAR-132] Isochrone-Berechnung für neuen SV fehlgeschlagen:', err)
    }
  }
...
// AAR-132: lokale OSRM+Seeded-Fallback-Funktion entfernt — Zentrale
// calculateIsochrone aus @/lib/isochrone/calculate-isochrone (HERE API) wird
// oben importiert und genutzt.
