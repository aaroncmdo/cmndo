...
// Vuln (vor Fix): assertKundeOwnsFall prueft nur `fallId`, der `terminId` kam roh vom
// Client -> ein Kunde konnte per fremdem terminId einen fremden SV-Termin ueberschreiben
// + verbindlich bestaetigen. Fix: Termin muss ueber EINE FK-Spalte (fall_id/claim_id/
// lead_id) zum ownership-geprueften Fall gehoeren. Mock-Muster: siehe reparatur-termin-vorschlag.test.
...
const h = vi.hoisted(() => ({
  ...
  terminBesitz: null as null | { fall_id: string | null; claim_id: string | null; lead_id: string | null },
  ...
  createAdminClient: vi.fn(() => ({
    from: vi.fn((table: string) => {
      if (table === 'gutachter_termine') {
        return {
          // Guard-Read: .select(...).eq('id', terminId).maybeSingle()
          select: vi.fn(() => ({
            eq: vi.fn(() => ({
              maybeSingle: vi.fn(async () => ({ data: h.terminBesitz, error: null })),
            })),
          })),
...
describe('waehleGegenvorschlagSlot — IDOR-Guard (Sec-Audit 07.07.)', () => {
  it('fremder terminId (keine FK-Spalte matcht den Fall) -> abgelehnt, KEIN Update/Bestaetigen', async () => {
    h.terminBesitz = { fall_id: 'fremd-fall', claim_id: 'fremd-claim', lead_id: 'fremd-lead' }
    const r = await waehleGegenvorschlagSlot('fall-1', 'fremder-termin', slot)
    expect(r.success).toBe(false)
    expect(r.error).toMatch(/nicht gefunden/i)
  ...
  it('eigener Termin via fall_id -> Update + Bestaetigen -> success', async () => {
    h.terminBesitz = { fall_id: 'fall-1', claim_id: null, lead_id: null }
  ...
  it('eigener Termin NUR via lead_id (fall_id/claim_id null) -> akzeptiert (schuetzt lead-gekeyte Termine)', async () => {
    h.terminBesitz = { fall_id: null, claim_id: null, lead_id: 'lead-1' }
  ...
  it('eigener Termin via claim_id -> akzeptiert', async () => {
    h.terminBesitz = { fall_id: null, claim_id: 'claim-1', lead_id: null }
