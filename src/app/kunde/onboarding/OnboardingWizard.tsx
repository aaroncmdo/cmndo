...
import {
  CheckIcon, CalendarIcon, FileTextIcon, SparklesIcon, FolderOpenIcon,
  AlertCircleIcon, InfoIcon, XIcon,
} from 'lucide-react'
import {
  completeOnboarding,
...
export default function OnboardingWizard({
  vorname, fall, claim, termin, pflichtDocs, abrechnungsweg, pflichtSlots = [],
  dokAnforderungen: dokAnforderungenProp = [], fahrzeugdaten = null,
}: {
  vorname: string
  fall: Fall | null
  ...
  /** 09.09.2026: die bereits bekannten Fahrzeug-/Halterdaten fuer den Fahrzeugschein-
   *  Schritt — damit der Kunde sie sieht und richtigstellen kann, statt sie neu zu tippen. */
  fahrzeugdaten?: Record<string, unknown> | null
}) {
  ...
  // setDocStatus entfiel mit dem toten Upload-Pfad; gelesen wird der Stand weiterhin.
  const [docStatus] = useState<Record<string, string>>(
  ...
  // Der Pflichtslot des Fahrzeugscheins — er entscheidet, ob der Schein schon vorliegt
  // (dann wird er anerkannt statt erneut verlangt) und wohin ein neues Foto gehoert.
  const fahrzeugscheinSlot = pflichtDocs.find((d) => d.slot_id === 'fahrzeugschein') ?? null
  ...
  // 09.09.2026: Hier standen handleFilesUpload / handleFileUpload / handleFreiUpload
  // samt zb1Result-State — seit dem Umbau auf die zentrale PflichtdokumenteSection
  // (CMM-33/35) rief sie NIEMAND mehr auf. Sie trugen einen zweiten, toten OCR-Pfad
  // neben dem Fahrzeugschein-Schritt; entfernt statt ihn ein drittes Mal zu pflegen.
          ...
          <div className="bg-white border border-claimondo-border rounded-3xl px-6 py-7 shadow-claimondo-md">
            ...
            {currentStep.id === 'welcome' && (
              <div>
                ...
                {/* 09.09.2026 (Aaron): der Fahrzeugschein gehoert an den Anfang — anerkennen,
                    wenn er vorliegt; sonst auslesen und die Werte bestaetigen lassen.
                    Der alte Block zeigte vier Werte rein lesend und legte nichts in die Akte. */}
                {fall?.id && (
                  <Zb1Karte
                    fallId={fall.id}
                    pflichtdokumentId={fahrzeugscheinSlot?.id ?? null}
                    liegtVor={fahrzeugscheinSlot?.status === 'hochgeladen'}
                    bekannteWerte={fahrzeugdaten}
                    onDatenGeaendert={() => router.refresh()}
                  />
                )}
