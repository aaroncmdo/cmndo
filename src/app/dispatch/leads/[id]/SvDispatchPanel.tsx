...
export default function SvDispatchPanel({
    ...
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-ios-xl p-5 space-y-3">
        ...
        <button
          ...
          disabled={pending}
          onClick={handleCancel}
          className="w-full text-xs font-medium px-3 py-2 rounded-ios-lg border border-claimondo-border text-claimondo-navy hover:bg-claimondo-bg disabled:opacity-50"
        >
  ...
  return (
    <div className="bg-white border border-claimondo-border rounded-ios-xl p-5 space-y-4">
      <div className="flex items-center justify-between">
      ...
      {!hardGateOk ? (
        <div className="space-y-2">
          <p className="text-xs text-claimondo-ondo flex items-start gap-2">
        ...
        <>
          ...
          {topSuggestions && topSuggestions.length > 0 && !showManual && (
            <div className="space-y-2">
              ...
              {extraSuggestions && extraSuggestions.length > 0 && (
                <div className="border-t border-claimondo-border pt-2 space-y-1.5">
                  <p className="text-[10px] text-claimondo-ondo uppercase font-medium">
                    Weitere Kandidaten
                  </p>
                    ...
                    {extraSuggestions.map((s) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-ios-lg border transition-colors ${
                            isSel
                        ...
                        >
                          ...
                          <div
                            className={`flex items-center gap-3 mt-1 text-[11px] ${
                              isSel ? 'text-white/80' : 'text-claimondo-ondo'
                            }`}
                          >
              ...
              {extraSuggestions && extraSuggestions.length === 0 && (
                <p className="text-[11px] text-claimondo-ondo/70 italic">
          ...
          {selectedSv && !topSuggestions?.some((t) => t.svId === selectedSv.svId) && !showManual && (
            <div className="space-y-3 border-t border-claimondo-border pt-4">
              ...
              {slotsLoading ? (
                <p className="text-[11px] text-claimondo-ondo/70 italic">Lade Slots …</p>
              ) : freeSlots.length > 0 ? (
          ...
          {showManual && (
            <div className="space-y-3 border-t border-claimondo-border pt-4">
              <div className="flex items-center justify-between">
                <p className="text-xs font-medium text-claimondo-navy">Manuelle Zeit-Auswahl</p>
                <button
                  type="button"
                  onClick={() => {
                    setShowManual(false)
                    ...
                    {([...(topSuggestions ?? []), ...(extraSuggestions ?? [])] as SvSuggestion[]).map((s) => {
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => setSelectedSv(s)}
                          className="w-full text-left px-3 py-2 rounded-ios-lg border bg-white border-claimondo-border hover:border-claimondo-ondo hover:bg-claimondo-bg transition-colors"
                        >
                          <span className="text-sm font-medium">{s.name}</span>
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <input
                      ...
                      className="px-3 py-2 border border-claimondo-border rounded-ios-lg text-sm"
                    />
                    <input
                      type="time"
                      value={startZeit}
      ...
      <Modal open={debugOpen} onClose={() => setDebugOpen(false)} noPadding hideCloseButton maxWidth={672} ariaLabel="SV-Matching Debug">
        <div className="max-h-[85vh] overflow-hidden flex flex-col">
            ...
            <div className="overflow-y-auto px-5 py-3 space-y-3">
              ...
              {debugData && (
                <>
                  <div className="text-xs text-claimondo-ondo bg-claimondo-bg rounded-ios-lg p-2">
                    ...
                    {debugData.results.map((r) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="mt-1 text-[10px] text-claimondo-ondo flex flex-wrap gap-x-2 gap-y-0.5">
