...
export default async function DispatchDashboard() {
  ...
  const stats = [
    { label: 'Neue Leads heute', value: newLeadsRes.count ?? 0, icon: UsersIcon, color: 'text-blue-600', bg: 'bg-blue-50', href: '/dispatch/leads' },
        ...
        <div className="bg-white rounded-ios-lg shadow-ios-md">
          ...
          <div className="divide-y divide-gray-50 max-h-[400px] overflow-y-auto">
            {recentLeads.map((lead) => (
              <Link key={lead.id} href={`/dispatch/leads/${lead.id}`} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50 transition-colors">
            ...
            {recentLeads.length === 0 && (
              <p className="px-5 py-8 text-sm text-gray-400 text-center">Keine Leads vorhanden</p>
            )}
          </div>
        </div>
          ...
          <div className="divide-y divide-gray-50 max-h-[400px] overflow-y-auto">
            {tasks.map((task) => (
              <div key={task.id} className="px-5 py-3 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-gray-900 truncate">{task.titel}</p>
                  <p className="text-xs text-gray-400">{task.faellig_am ? new Date(task.faellig_am).toLocaleDateString('de-DE') : ''}</p>
                </div>
            ...
            {tasks.length === 0 && (
              <p className="px-5 py-8 text-sm text-gray-400 text-center">Keine offenen Tasks</p>
            )}
          </div>
        </div>
      </div>
