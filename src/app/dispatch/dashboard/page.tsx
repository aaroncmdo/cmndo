...
export default async function DispatchDashboard() {
  ...
  if (newLeadsRes.count) seg.push({ t: `${newLeadsRes.count} neue Leads` })
  if (openRueckrufeRes.count) seg.push({ t: `${openRueckrufeRes.count} Rückrufe offen` })
  if (overdueRueckrufe) seg.push({ t: `${overdueRueckrufe} überfällig`, danger: true })
  if (tasks.length) seg.push({ t: `${tasks.length} Tasks` })

  return (
    <div className="space-y-5">
      <div>
        <div className="flex flex-col gap-0.5 sm:flex-row sm:items-baseline sm:justify-between">
          <h1 className="text-heading-lg font-bold text-claimondo-navy">Dispatch</h1>
        ...
        <p className="mt-1 text-body-sm text-claimondo-ondo">
          {seg.length ? (
            ...
            'Ruhiger Moment — keine offenen Rückrufe.'
          )}
        </p>
      </div>
      ...
      <StatBar
        items={[
          ...
          { label: 'Offene Rückrufe', value: openRueckrufeRes.count ?? 0, icon: PhoneIcon, href: '/dispatch/rueckrufe', tone: overdueRueckrufe ? 'warning' : 'default' },
          { label: 'FlowLinks heute', value: flowLinksRes.count ?? 0, icon: LinkIcon, href: '/dispatch/leads' },
        ]}
      />

      {/* Rückrufe-Timeline: chronologische Liste, Click → Rückrufe-Liste mit Auto-Open-Popover */}
      <div className="bg-white rounded-ios-md shadow-ios-md border border-claimondo-navy/[0.06]">
