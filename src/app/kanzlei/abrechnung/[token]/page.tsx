        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-2xl shadow p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Bereits bezahlt</h1>
                <p className="text-gray-500">Diese Rechnung wurde am {abrechnung.bezahlt_am ? new Date(abrechnung.bezahlt_am as string).toLocaleDateString('de-DE') : '—'} bezahlt.</p>
              </div>
            </div>
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {kanzlei && (
          <div className="bg-white rounded-2xl shadow p-6">
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Rechnungsempfaenger</h2>
            <p className="font-bold text-gray-900">{kanzlei.name}</p>
            {kanzlei.ansprechpartner && <p className="text-gray-600">{kanzlei.ansprechpartner}</p>}
            {kanzlei.adresse && <p className="text-gray-500 text-sm">{kanzlei.adresse}</p>}
            {kanzlei.ust_id && <p className="text-gray-500 text-sm">USt-ID: {kanzlei.ust_id}</p>}
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow overflow-hidden">
          <div className="p-6 border-b">
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Positionen</h2>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-medium text-gray-600">Fall-Nr.</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600">Kunde</th>
                  <th className="text-left px-4 py-3 font-medium text-gray-600">Vollmacht unterz.</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {(positionen ?? []).map((pos) => (
                  <tr key={pos.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-gray-500">{pos.position_nr}</td>
                    <td className="px-4 py-3 font-mono text-xs text-gray-700">{pos.fall_nr ?? '—'}</td>
                    <td className="px-4 py-3 text-gray-900">{pos.kunde_name}</td>
