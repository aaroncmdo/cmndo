    ...
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-zinc-800 rounded-xl p-1">
        <button
          ...
          className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
            tab === 'email' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
            tab === 'telefon' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
            tab === 'google' ? 'bg-zinc-700 text-white' : 'text-zinc-500 hover:text-zinc-300'
          }`}
        >
      ...
      {tab === 'email' && (
        <form action={loginAction} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="password" className="text-sm font-medium text-zinc-300">Passwort</label>
            <input
              id="password"
              name="password"
              type="password"
      ...
      {tab === 'telefon' && (
        <div className="flex flex-col gap-4">
          {phoneStep === 'enter' ? (
            <>
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-zinc-300">Telefonnummer</label>
                <input
                  type="tel"
                  value={phone}
              ...
              {phoneError && (
                <p className="text-sm text-red-400 rounded-xl bg-red-950/50 border border-red-900 px-4 py-3">{phoneError}</p>
              )}
              <button
                type="button"
            ...
            <>
              <p className="text-zinc-400 text-sm">Code gesendet an <span className="text-white">{phone}</span></p>
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-zinc-300">6-stelliger Code</label>
                <input
                  type="text"
                  value={otp}
                  ...
                  placeholder="123456"
                  maxLength={6}
                  className="w-full px-4 py-3 rounded-xl border border-zinc-700 bg-zinc-800 text-white placeholder-zinc-500 text-sm text-center tracking-[0.5em] font-mono focus:outline-none focus:border-zinc-500 focus:ring-2 focus:ring-zinc-700 transition-all"
                />
              </div>
              {phoneError && (
                <p className="text-sm text-red-400 rounded-xl bg-red-950/50 border border-red-900 px-4 py-3">{phoneError}</p>
      ...
      {tab === 'google' && (
        <div className="flex flex-col gap-4">
          <p className="text-zinc-400 text-sm text-center">Mit Ihrem Google-Konto anmelden</p>
          <button
            type="button"
            onClick={handleGoogle}
            className="w-full py-3.5 rounded-xl bg-white hover:bg-zinc-200 text-zinc-950 font-semibold text-sm active:scale-[0.98] transition-all flex items-center justify-center gap-3"
          >
