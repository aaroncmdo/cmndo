...
export default function LoginClient({
  ...
  return (
    <div>
      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-[#f8f9fb] rounded-xl p-1">
        <button
          type="button"
          onClick={() => setTab('email')}
          className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
            tab === 'email' ? 'bg-[#1E3A5F] text-white' : 'text-claimondo-ondo hover:text-claimondo-navy'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
            tab === 'telefon' ? 'bg-[#1E3A5F] text-white' : 'text-claimondo-ondo hover:text-claimondo-navy'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
            tab === 'google' ? 'bg-[#1E3A5F] text-white' : 'text-claimondo-ondo hover:text-claimondo-navy'
          }`}
        >
        ...
        <form action={loginAction} className="flex flex-col gap-4">
          ...
          <div className="flex items-start gap-2">
            <input
              ...
              className="mt-0.5 w-4 h-4 rounded border-claimondo-border text-[#1E3A5F] focus:ring-2 focus:ring-[#1E3A5F]"
            />
          ...
          <Link
            ...
            className="text-center text-xs text-claimondo-ondo hover:text-[#1E3A5F] transition-colors -mt-1"
          >
            Passwort vergessen?
          </Link>
        </form>
            ...
            <>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  placeholder="123456"
                  maxLength={6}
                  className="w-full px-4 py-3 rounded-xl border border-claimondo-border bg-[#f8f9fb] text-claimondo-navy placeholder-zinc-500 text-sm text-center tracking-[0.5em] font-mono focus:outline-none focus:border-zinc-500 focus:ring-2 focus:ring-zinc-700 transition-all"
                />
              </div>
              {phoneError && (
                <p className="text-sm text-red-400 rounded-xl bg-red-50/50 border border-red-200 px-4 py-3">{phoneError}</p>
