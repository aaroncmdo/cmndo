...
function EmailSubmitButton() {
  ...
  return (
    <LoadingButton
      ...
      className="w-full py-3.5 rounded-ios-md bg-claimondo-shield hover:bg-claimondo-ondo text-white font-semibold text-sm active:scale-[0.98] transition-all mt-1 disabled:opacity-60 disabled:cursor-not-allowed"
    >
      Einloggen
    </LoadingButton>
  )
...
export default function LoginClient({
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      throw new Error('Keine Weiterleitungs-URL von Google erhalten')
    } catch (err) {
  ...
  return (
    <div>
      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-claimondo-bg rounded-ios-md p-1">
        <button
          type="button"
          onClick={() => setTab('email')}
          className={`flex-1 py-2 px-2 rounded-ios-sm text-xs font-medium transition-colors flex items-center justify-center gap-1.5 ${
        ...
        <form action={loginAction} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              required
              autoComplete="email"
              className="w-full px-4 py-3 rounded-ios-md border border-claimondo-border bg-claimondo-bg text-claimondo-navy placeholder-claimondo-shield text-sm focus:outline-none focus:border-claimondo-ondo focus:ring-2 focus:ring-claimondo-ondo transition-all"
            />
          </div>
          <div className="flex flex-col gap-1.5">
      ...
      {tab === 'google' && (
        <div className="flex flex-col gap-4">
          ...
          <button
            ...
            className="w-full py-3.5 rounded-ios-md bg-claimondo-shield hover:bg-claimondo-ondo text-white font-semibold text-sm active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-60 disabled:cursor-not-allowed"
          >
