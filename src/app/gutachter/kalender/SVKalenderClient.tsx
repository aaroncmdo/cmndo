...
export default function SVKalenderClient({
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {/* Sticky Topbar — Mobile-Stack: links Titel + (kompakter) GCal-Indikator,
          darunter (sm: rechts) der lange GCal-Status + die Wochen-Pagination. */}
      <div className="flex-shrink-0 bg-white border-b border-claimondo-border px-4 py-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center justify-between gap-2 min-w-0">
          ...
          {gcalConnected ? (
            ...
            <a
              ...
              className="sm:hidden shrink-0 inline-flex items-center justify-center w-9 h-9 bg-white border border-claimondo-border hover:shadow-md rounded-ios-lg transition-shadow"
        ...
        <div className="flex items-center gap-2 justify-between sm:justify-end">
          {gcalConnected ? (
            ...
            <a
              href="/api/auth/google/connect?return=/gutachter/kalender"
              className="hidden sm:flex items-center gap-2 bg-white border border-claimondo-border hover:shadow-md text-sm font-medium px-3 py-1.5 rounded-ios-lg transition-shadow"
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <button onClick={() => setCurrentDate(subWeeks(currentDate, 1))} className="px-2.5 sm:px-3 py-1.5 text-xs text-claimondo-ondo hover:text-claimondo-navy hover:bg-claimondo-bg rounded-ios-lg transition-colors">← <span className="hidden sm:inline">Zurück</span></button>
            <button onClick={() => setCurrentDate(new Date())} className="px-2.5 sm:px-3 py-1.5 text-xs text-claimondo-navy hover:text-claimondo-navy hover:bg-claimondo-bg rounded-ios-lg transition-colors font-medium">Heute</button>
            <button onClick={() => setCurrentDate(addWeeks(currentDate, 1))} className="px-2.5 sm:px-3 py-1.5 text-xs text-claimondo-ondo hover:text-claimondo-navy hover:bg-claimondo-bg rounded-ios-lg transition-colors"><span className="hidden sm:inline">Weiter</span> →</button>
          </div>
        </div>
      </div>
                    ...
                    {(verlegteSlots ?? [])
                      ...
                      .map((v) => {
                        ...
                        return (
                          <div
                            key={`verlegt-${v.id}`}
                            className="px-2 py-1 rounded-ios-lg text-[10px] leading-tight bg-claimondo-border/30 text-claimondo-ondo/70 border border-claimondo-border/60 italic"
                  ...
                  {gegSlots.map((slot, idx) => (
                    <div key={idx} className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-claimondo-bg border border-claimondo-border rounded-ios-lg px-2 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--brand-secondary)]"
                      />
                      <input
                        type="time"
                        value={slot.uhrzeit}
