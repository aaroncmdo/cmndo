...
export default function StopListItem({ stop, variant }: StopListItemProps) {
  ...
  return (
    <div
      className={
        erledigt
          ? 'rounded-lg bg-white/5 p-3 text-xs text-gray-400 flex items-start gap-2'
          : 'rounded-lg bg-white/10 p-3 text-xs text-gray-200 flex items-start gap-2'
      }
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-gray-400 truncate">{stop.adresse}</p>
      </div>
    </div>
  )
}
