            ...
            : termine.map((t, i) => (
              <div key={t.id}>
                <div className="bg-white border border-gray-200 rounded-lg p-3">
                  ...
                  <div className="flex gap-2 mt-2"><button onClick={() => startNav(t)} className="flex items-center gap-1 bg-green-50 hover:bg-green-100 text-green-700 text-xs font-medium px-3 py-1.5 rounded-lg"><NavigationIcon className="w-3 h-3" /> Route starten</button>{t.telefon && <a href={`tel:${t.telefon}`} className="flex items-center gap-1 bg-[#4573A2]/5 hover:bg-[#4573A2]/10 text-[#1E3A5F] text-xs font-medium px-3 py-1.5 rounded-lg"><PhoneIcon className="w-3 h-3" /> Anrufen</a>}<Link href={`/gutachter/fall/${t.id}`} className="text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5">Details</Link></div>
                </div>
