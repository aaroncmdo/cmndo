...
export default async function GutachterDashboard() {
  ...
  const [todayRes, neueRes, tasksRes, erledigtRes, faelligRes] = await Promise.all([
    ...
    supabase.from('faelle')
      .select('id, fall_nummer, status, schadens_adresse, schadens_plz, schadens_ort, lead_id, kennzeichen, schadenfall_typ, created_at')
    ...
    // Nächstes Gutachten fällig
    supabase.from('faelle')
      .select('id, fall_nummer, lead_id, sv_termin')
      .eq('sv_id', sv.id).not('sv_termin', 'is', null).lt('sv_termin', now.toISOString())
      .is('gutachten_eingegangen_am', null).not('status', 'in', '("abgeschlossen","storniert")')
      .order('sv_termin', { ascending: true }).limit(1),
  ])

  const allLeadIds = [...new Set([...(todayRes.data ?? []), ...(neueRes.data ?? []), ...(faelligRes.data ?? [])].map(f => f.lead_id).filter(Boolean) as string[])]
  ...
  if (allLeadIds.length) {
    ...
    for (const l of leads ?? []) leadMap[l.id] = { name: [l.vorname, l.nachname].filter(Boolean).join(' ') || '—', telefon: l.telefon, email: l.email }
  }
  ...
  const termine = (todayRes.data ?? []).map(f => ({
    ...
    kunde: leadMap[f.lead_id ?? '']?.name ?? '—', telefon: leadMap[f.lead_id ?? '']?.telefon ?? null, email: leadMap[f.lead_id ?? '']?.email ?? null,
    adresse: [f.schadens_adresse, f.schadens_plz, f.schadens_ort].filter(Boolean).join(', '),
    kennzeichen: (f.kennzeichen as string) ?? null, fahrzeug: [f.fahrzeug_hersteller, f.fahrzeug_modell].filter(Boolean).join(' ') || null,
    schadentyp: (f.schadenfall_typ as string) ?? null, status: f.status as string,
  }))

  const neueKunden = (neueRes.data ?? []).map(f => ({
    id: f.id as string, fallNr: f.fall_nummer as string | null,
    kunde: leadMap[f.lead_id ?? '']?.name ?? '—', kennzeichen: (f.kennzeichen as string) ?? null,
  ...
  const tasks = (tasksRes.data ?? []).map(t => ({ id: t.id as string, titel: t.titel as string, faellig: t.faellig_am as string | null, fallId: t.fall_id as string | null }))

  const faelligFall = (faelligRes.data ?? [])[0] ?? null
  const gutachtenFaellig = faelligFall ? { id: faelligFall.id as string, fallNr: faelligFall.fall_nummer as string | null, kunde: leadMap[faelligFall.lead_id ?? '']?.name ?? '—' } : null

  const hour = now.getHours()
  const greeting = `${hour < 12 ? 'Guten Morgen' : hour < 18 ? 'Guten Tag' : 'Guten Abend'}${profile?.vorname ? ` ${profile.vorname}` : ''}`

  return (
    <GutachterFieldApp
      greeting={greeting} datum={now.toLocaleDateString('de-DE', { weekday: 'long', day: '2-digit', month: '2-digit', year: 'numeric' })}
      termine={termine} neueKunden={neueKunden} tasks={tasks} routeUrl={routeUrl} gutachtenFaellig={gutachtenFaellig}
      ...
      svLat={sv.standort_lat ? Number(sv.standort_lat) : null} svLng={sv.standort_lng ? Number(sv.standort_lng) : null}
