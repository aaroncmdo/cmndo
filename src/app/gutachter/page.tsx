    ...
    <div className="h-full overflow-hidden flex flex-col">
      ...
      <div className="h-10 flex items-center justify-between px-4 bg-white border-b border-gray-200 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 text-[10px] font-medium">
          <span className="bg-[#4573A2]/5 text-[#1E3A5F] px-2 py-0.5 rounded-full">{termine.length} Termine</span>
        ...
        {mode === 'overview' && <div className="grid grid-cols-1 lg:grid-cols-2 h-full">
            ...
            : termine.map((t, i) => (
              <div key={t.id}>
                <div className="bg-white border border-gray-200 rounded-lg p-3">
                  <div className="flex items-center gap-3"><span className="text-lg font-bold text-[#4573A2] tabular-nums w-14 shrink-0">{t.uhrzeit}</span><div className="flex-1 min-w-0"><span className="text-sm font-semibold text-gray-900">{t.kunde}</span>{t.kennzeichen && <span className="ml-2 text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded">{t.kennzeichen}</span>}<p className="text-xs text-gray-500 truncate mt-0.5">{t.adresse}</p>{t.schadentyp && <span className="text-[9px] bg-[#4573A2]/5 text-[#4573A2] px-1 py-0.5 rounded mt-0.5 inline-block">{t.schadentyp.toUpperCase()}</span>}</div></div>
                  <div className="flex gap-2 mt-2"><button onClick={() => startNav(t)} className="flex items-center gap-1 bg-green-50 hover:bg-green-100 text-green-700 text-xs font-medium px-3 py-1.5 rounded-lg"><NavigationIcon className="w-3 h-3" /> Route starten</button>{t.telefon && <PhoneButton nummer={t.telefon} variant="inline" label="Anrufen" className="!flex !items-center !gap-1 !bg-[#4573A2]/5 hover:!bg-[#4573A2]/10 !text-[#1E3A5F] !text-xs !font-medium !px-3 !py-1.5 !rounded-lg" />}<Link href={`/gutachter/fall/${t.id}`} className="text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5">Details</Link></div>
                </div>
          ...
          <div className="overflow-y-auto p-4 space-y-3 hidden lg:block">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="bg-[#4573A2]/5 border border-[#4573A2]/20 rounded-lg p-2 text-center"><p className="text-sm font-bold text-[#4573A2]">{finanz.eingegangen - finanz.leadpreise}€</p><p className="text-[9px] text-gray-500">Saldo</p></div>
            </div>
          </div>
        </div>}
...
const TL_CFG: Record<string, { dot: string; ico: typeof CalendarIcon; fg: string }> = {
  termin:       { dot: 'bg-[#4573A2]', ico: CalendarIcon,       fg: 'text-[#4573A2]' },
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-[#0D1B3E]">Tagesverlauf</h3>
        {isPast && <span className="text-[9px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full font-medium">Vergangen</span>}
        {isFuture && <span className="text-[9px] bg-[#4573A2]/10 text-[#4573A2] px-2 py-0.5 rounded-full font-medium">Geplant</span>}
      </div>
