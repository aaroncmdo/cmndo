...
import { NavigationIcon, PhoneIcon, CameraIcon, CheckIcon, MapPinIcon, ClockIcon, FolderOpenIcon, WalletIcon, UploadIcon } from 'lucide-react'
...
export default function GutachterFieldApp({ greeting, datum, termine, neueAuftraege, tasks, routeUrl, stats, svLat, svLng, mapsKey }: {
  ...
  // Mobile tab for single-column view
  const [mobileTab, setMobileTab] = useState<'route' | 'kunde' | 'uploads'>('route')

  function startNavigation(t: Termin) {
    setActiveFall(t)
    setMode('navigation')
    setFotos({})
    setFin('')
  ...
  function arriveOnsite() {
    setMode('onsite')
    // Timeline + WhatsApp would be triggered server-side
  }
  ...
  async function handleFotoUpload(slot: string, file: File) {
    ...
    try {
      ...
      await supabase.from('dokumente').insert({
        fall_id: activeFall.id, typ: 'schadensfoto', datei_url: publicUrl,
        datei_name: `${slot}.${ext}`, datei_groesse: file.size,
        kategorie: 'schadensfotos', hochgeladen_von_rolle: 'sachverstaendiger', quelle: 'gutachter-app',
      })
  ...
  async function completeVisit() {
    ...
    try {
      ...
      await supabase.from('timeline').insert({
        fall_id: activeFall.id, typ: 'system',
        titel: 'Besichtigung abgeschlossen',
        beschreibung: `${FOTO_SLOTS.filter(s => fotos[s]).length} Fotos, FIN: ${fin || '—'}, KM: ${km || '—'}. ${notizen}`.trim(),
      })
      setMode('overview')
      setActiveFall(null)
      router.refresh()
  ...
  const mapsEmbedUrl = activeFall && mapsKey && svLat && svLng
    ? `https://www.google.com/maps/embed/v1/directions?key=${mapsKey}&origin=${svLat},${svLng}&destination=${encodeURIComponent(activeFall.adresse)}&mode=driving`
    : null
    ...
    <div style={{ height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="flex items-center justify-between px-4 py-2 shrink-0 border-b border-gray-100">
        <div className="flex items-center gap-3">
          <h1 className="text-sm font-semibold text-gray-900">{greeting}</h1>
          <span className="text-gray-400 text-xs">{datum}</span>
        </div>
        {mode !== 'overview' && (
          <button onClick={() => { setMode('overview'); setActiveFall(null) }}
            className="text-xs text-gray-500 hover:text-gray-700 bg-gray-100 px-3 py-1 rounded-lg">
            ← Übersicht
          </button>
        )}
      </div>

      {/* Mobile tabs (only in nav/onsite mode, <768px) */}
      {mode !== 'overview' && (
        <div className="flex lg:hidden border-b border-gray-200 shrink-0">
          {(['route', 'kunde', 'uploads'] as const).map(t => (
            <button key={t} onClick={() => setMobileTab(t)}
              ...
              {t === 'route' ? 'Route' : t === 'kunde' ? 'Kunde' : 'Uploads'}
            </button>
          ))}
        </div>
      )}
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        {mode === 'overview' && (
          <>
            {/* LEFT: Tagesplan */}
            <div className="flex-1 min-w-0 overflow-y-auto p-3 space-y-2 border-r border-gray-100">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-semibold text-gray-700">{termine.length} Termine heute</span>
                {routeUrl && (
                  <a href={routeUrl} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1 bg-green-600 hover:bg-green-500 text-white text-[10px] font-medium px-2.5 py-1 rounded-lg">
                ...
                <div key={t.id} className="bg-white border border-gray-200 rounded-xl p-3 hover:shadow-sm transition-shadow">
                  <div className="flex items-center gap-3">
                    <div className="shrink-0 text-center w-12">
                      <p className="text-blue-600 text-sm font-bold tabular-nums">{t.uhrzeit}</p>
                    </div>
                    ...
                    <button onClick={() => startNavigation(t)}
                      className="shrink-0 flex items-center gap-1 bg-green-50 hover:bg-green-100 text-green-700 text-[10px] font-medium px-2.5 py-1.5 rounded-lg">
            ...
            {/* RIGHT: Aufträge + Tasks + KPIs */}
            <div className="w-[45%] hidden lg:block overflow-y-auto p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-white border border-gray-200 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-gray-900">{stats.faelle}/{stats.maxFaelle}</p>
                  <p className="text-[10px] text-gray-500">Fälle</p>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-gray-900">{stats.erledigtMonat}</p>
                  <p className="text-[10px] text-gray-500">Monat</p>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-gray-900">{Math.round((stats.faelle / Math.max(1, stats.maxFaelle)) * 100)}%</p>
                  <p className="text-[10px] text-gray-500">Auslastung</p>
                </div>
                <div className="bg-white border border-gray-200 rounded-xl p-3 text-center">
                  <p className="text-lg font-bold text-gray-900">{stats.guthaben}€</p>
        ...
        {/* ═══ MODUS 2: NAVIGATION + MODUS 3: VOR-ORT ═══ */}
        {mode !== 'overview' && activeFall && (
          <>
            {/* LEFT: Map (nav) or Uploads (onsite) */}
            <div className={`flex-1 min-w-0 flex flex-col ${mobileTab !== 'route' && mobileTab !== 'uploads' ? 'hidden lg:flex' : ''}`}>
              {mode === 'navigation' && (mobileTab === 'route' || typeof window !== 'undefined' && window.innerWidth >= 1024) && (
                <>
                  {mapsEmbedUrl ? (
                    <iframe src={mapsEmbedUrl} className="flex-1 w-full border-0" allowFullScreen loading="lazy" />
                  ) : (
                    <div className="flex-1 bg-gray-100 flex items-center justify-center text-gray-400 text-sm">Karte nicht verfügbar</div>
                  )}
                  <div className="shrink-0 p-3 border-t border-gray-200 flex items-center gap-2">
                    {navUrl && (
                      <a href={navUrl} target="_blank" rel="noopener noreferrer"
                        className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-500 text-white text-sm font-medium py-2.5 rounded-lg">
                        <NavigationIcon className="w-4 h-4" /> In Google Maps öffnen
                      </a>
                    )}
                    <button onClick={arriveOnsite}
                      className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium py-2.5 rounded-lg">
                      <MapPinIcon className="w-4 h-4" /> Bin angekommen
                    </button>
                  </div>
                </>
              )}

              {mode === 'onsite' && (mobileTab === 'uploads' || mobileTab === 'route' || typeof window !== 'undefined' && window.innerWidth >= 1024) && (
                <div className="flex-1 overflow-y-auto p-3 space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-gray-900">Vor-Ort Dokumentation</p>
                    <span className="text-xs text-blue-600 font-medium">{fotosCount + (fin.length === 17 ? 1 : 0) + (km ? 1 : 0)} / {FOTO_SLOTS.length + 2}</span>
                  </div>
                  <div className="h-1.5 bg-gray-200 rounded-full">
                    <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${((fotosCount + (fin.length === 17 ? 1 : 0) + (km ? 1 : 0)) / (FOTO_SLOTS.length + 2)) * 100}%` }} />
                  </div>

                  {/* Foto-Slots */}
                  <div className="grid grid-cols-2 gap-2">
                    {FOTO_SLOTS.map(s => (
                      <label key={s} className={`flex items-center justify-center gap-2 py-4 rounded-xl border-2 border-dashed cursor-pointer transition-colors min-h-[48px] ${
                        fotos[s] ? 'border-green-300 bg-green-50 text-green-600' : 'border-gray-200 bg-gray-50 text-gray-500 hover:border-blue-300'
                      }`}>
                        {fotos[s] ? <CheckIcon className="w-5 h-5" /> : <CameraIcon className="w-5 h-5" />}
                        <span className="text-sm font-medium">{s}</span>
                        <input type="file" accept="image/*" capture="environment" className="hidden"
                          onChange={e => { if (e.target.files?.[0]) handleFotoUpload(s, e.target.files[0]) }} disabled={uploading} />
                      </label>
                    ))}
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-3 space-y-2">
                    <div>
                      <label className="text-xs text-gray-500 block mb-1">FIN (17 Zeichen)</label>
                      <input type="text" value={fin} onChange={e => setFin(e.target.value.toUpperCase().slice(0, 17))} maxLength={17}
                        placeholder="WVWZZZ3CZWE123456" className="w-full bg-white border border-gray-300 text-sm font-mono rounded-lg px-3 py-2 tracking-wider" />
                    </div>
                    <div>
                      <label className="text-xs text-gray-500 block mb-1">Kilometerstand</label>
                  ...
                  <button onClick={completeVisit} disabled={completing || fotosCount < 4}
                    className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-sm font-medium py-3 rounded-xl min-h-[48px]">
                    {completing ? 'Speichert...' : 'Besichtigung abschliessen'}
                  </button>
                  {fotosCount < 4 && <p className="text-gray-400 text-[10px] text-center">Min. 4 Fotos benötigt</p>}
                </div>
              )}
            </div>

            {/* RIGHT: Kunden-Info */}
            <div className={`w-[45%] lg:block overflow-y-auto p-3 space-y-2 border-l border-gray-100 ${mobileTab !== 'kunde' ? 'hidden' : 'block w-full'}`}>
              <div className="bg-white border border-gray-200 rounded-xl p-3">
                ...
                {activeFall.telefon && (
                  <a href={`tel:${activeFall.telefon}`} className="flex items-center gap-1.5 text-blue-600 text-sm mt-1">
                    <PhoneIcon className="w-3.5 h-3.5" /> {activeFall.telefon}
                  </a>
                )}
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-3">
                <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Fahrzeug</p>
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-3">
                ...
                <p className="text-xs text-gray-500 mt-1">Termin: {activeFall.uhrzeit}</p>
              </div>

              {activeFall.telefon && (
                <a href={`tel:${activeFall.telefon}`}
                  className="flex items-center justify-center gap-2 bg-green-50 hover:bg-green-100 text-green-700 text-sm font-medium py-2.5 rounded-xl w-full">
                  <PhoneIcon className="w-4 h-4" /> Kunden anrufen
                </a>
              )}
            </div>
          </>
