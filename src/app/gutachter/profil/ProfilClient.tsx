  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-6 max-w-full">
        ...
        {/* Kalender-Verbindung
            AAR-707: Hardcode-Toasts entfernt. Google nutzt jetzt den echten
            OAuth-Flow aus AAR-google-cal-drift (Tokens in profiles.google_*),
            Outlook bleibt als „Bald verfügbar"-Disabled-Knopf bis OAuth
            implementiert ist — kein irreführender „wird konfiguriert"-Toast mehr. */}
        <div className="bg-white rounded-2xl p-6 border border-gray-200 mt-5">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-800 text-sm font-medium">
                  {googleConnected ? 'Google Kalender verbunden' : 'Nicht verbunden'}
                </p>
                {googleConnected && (
                  <p className="text-gray-500 text-xs mt-0.5">
                    FreeBusy + Event-Sync aktiv. Termine werden automatisch
                    in deinen Google-Kalender geschrieben.
                  </p>
                )}
              </div>
              <span className={`w-2.5 h-2.5 rounded-full ${googleConnected ? 'bg-green-500' : 'bg-zinc-400'}`} />
            </div>

            {!googleConnected && (
              <div className="flex gap-2">
                <a
                  href="/api/auth/google/connect?return=/gutachter/profil"
                  className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-white hover:shadow text-gray-800 border border-gray-300 text-center transition-shadow inline-flex items-center justify-center gap-2"
                >
            ...
            {/* AAR-717: Link zur neuen zentralen Kalender-Einstellungs-Seite
                (Google + Apple iCloud/CalDAV + Status pro Provider + Fehler-
                Anzeige + Disconnect). */}
            <a
              href="/gutachter/einstellungen/kalender"
              className="block text-xs text-[#4573A2] hover:text-[#0D1B3E] underline mt-1"
            >
              Alle Kalender-Einstellungen öffnen →
            </a>
