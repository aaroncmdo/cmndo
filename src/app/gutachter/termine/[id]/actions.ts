      ...
      const sendKundeWa = async () => {
        ...
        await sendNachricht({
          ...
          text: [
            ...
            `Unser Team wird sich schnellstmöglich mit Ihnen in Verbindung setzen, um einen neuen Termin zu vereinbaren.`,
            `Ihr Portal: ${portalLink}`,
          ].join('\n\n'),
    ...
    import('@/lib/whatsapp/send').then(({ sendNachricht }) => {
      const sendKundeWa = async () => {
        ...
        await sendNachricht({
          ...
          phone: kundeTelefon,
          text: [
            `Hallo ${kundeVorname || 'Kunde'}, Ihr Gutachter hat ${slots.length} neue Terminvorschläge für Sie:`,
            erstDatum ? `Erster Vorschlag: ${erstDatum} Uhr` : null,
            slots.length > 1 ? `+ ${slots.length - 1} weitere Option${slots.length > 2 ? 'en' : ''}` : null,
            '',
            `Bitte bestätigen Sie Ihren Wunschtermin in Ihrem Portal:`,
            portalLink,
