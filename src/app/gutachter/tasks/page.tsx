...
const PRIO_COLORS: Record<string, string> = {
  kritisch: 'bg-red-950 text-red-300 border-red-800',
  dringend: 'bg-amber-950 text-amber-300 border-amber-800',
    ...
    <div className="px-4 py-6 sm:py-8">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-semibold text-white">Meine Tasks</h1>
          <span className="ml-auto bg-zinc-800 text-zinc-300 text-xs font-medium px-2.5 py-1 rounded-full">
        ...
        {ueberfaellige.length > 0 && (
          <div className="bg-red-950/50 border border-red-800/50 rounded-2xl p-4 flex items-center gap-3">
        ...
        {offeneTasks.length === 0 ? (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-12 text-center">
            ...
            <p className="text-zinc-400 text-sm">Keine offenen Tasks. Alles erledigt!</p>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  className={`bg-zinc-900 border rounded-2xl p-4 ${
                    dl?.overdue ? 'border-red-800/60' : 'border-zinc-800'
                  }`}
                >
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="text-white text-sm font-medium">{task.titel}</h3>
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded border ${PRIO_COLORS[prio]}`}>
                          {PRIO_LABELS[prio]}
                        </span>
                      </div>
                      {task.beschreibung && (
                        <p className="text-zinc-500 text-xs mt-1 line-clamp-2">{task.beschreibung}</p>
                      )}
                      <div className="flex items-center gap-3 mt-2 text-xs text-zinc-500">
                        ...
                        {dl && (
                          <span className={`flex items-center gap-1 ${dl.overdue ? 'text-red-400' : 'text-zinc-400'}`}>
