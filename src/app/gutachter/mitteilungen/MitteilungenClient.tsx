...
const TYP_COLOR: Record<string, string> = {
  neuer_auftrag: 'text-blue-400 bg-blue-950',
  termin_bestaetigt: 'text-green-400 bg-green-950',
  termin_geaendert: 'text-amber-400 bg-amber-950',
  kunde_dokument_hochgeladen: 'text-sky-400 bg-sky-950',
  kunde_chat_nachricht: 'text-violet-400 bg-violet-950',
  vorschaden_warnung: 'text-red-400 bg-red-950',
  gutachten_erinnerung: 'text-amber-400 bg-amber-950',
  qc_bestanden: 'text-green-400 bg-green-950',
  qc_nachbesserung: 'text-red-400 bg-red-950',
  kanzlei_as_gesendet: 'text-purple-400 bg-purple-950',
  kanzlei_regulierung: 'text-purple-400 bg-purple-950',
  kanzlei_zahlung: 'text-emerald-400 bg-emerald-950',
  paket_fast_voll: 'text-amber-400 bg-amber-950',
  guthaben_niedrig: 'text-red-400 bg-red-950',
}
    ...
    <div className="px-4 py-6 sm:py-8">
      <div className="max-w-3xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-white">Mitteilungen</h1>
        ...
        {filtered.length === 0 ? (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-12 text-center">
            <BellIcon className="w-8 h-8 text-zinc-700 mx-auto mb-3" />
            <p className="text-zinc-500 text-sm">Keine Mitteilungen.</p>
          </div>
        ) : (
          <div className="space-y-2">
            {filtered.map(m => {
              const Icon = TYP_ICON[m.typ] ?? BellIcon
              const colorCls = TYP_COLOR[m.typ] ?? 'text-zinc-400 bg-zinc-800'
              const [iconColor, iconBg] = colorCls.split(' ')

              const card = (
                <div
                  className={`flex items-start gap-4 bg-zinc-900 border rounded-2xl p-4 transition-colors ${
                    !m.gelesen
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <p className={`text-sm font-medium ${!m.gelesen ? 'text-white' : 'text-zinc-400'}`}>
                        {m.titel}
                        {m.dringend && (
                          <span className="ml-2 text-xs text-red-400 font-semibold">DRINGEND</span>
                        )}
                      </p>
                      <span className="shrink-0 text-xs text-zinc-600">{formatTime(m.created_at)}</span>
                    </div>
                    <p className="text-zinc-500 text-xs mt-1 line-clamp-2">{m.nachricht}</p>
                  </div>
                  {!m.gelesen && (
