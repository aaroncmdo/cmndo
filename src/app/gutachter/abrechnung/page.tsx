  ...
  return (
    <div className="px-4 py-8">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-white">Abrechnung</h1>
          <p className="text-zinc-500 text-sm mt-0.5">Uebersicht Ihrer Abrechnungen und Pakete</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          ...
          <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-zinc-400 text-sm font-medium">Guthaben</p>
              </div>
            </div>
            <p className="text-3xl font-bold text-white tabular-nums">{guthaben.toLocaleString('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} EUR</p>
            {guthaben <= 0 && <p className="text-zinc-600 text-xs mt-1">Kein Guthaben vorhanden</p>}
          </div>
          ...
          <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-zinc-400 text-sm font-medium">Paket-Auslastung</p>
                <p className="text-zinc-600 text-xs">{paketLabel}</p>
              </div>
            </div>
            <div className="flex items-end justify-between mb-2">
              ...
              <p className="text-zinc-500 text-sm tabular-nums">{auslastungProzent}%</p>
            </div>
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-lg font-semibold text-white">Abgerechnete Faelle</h2>
            <span className="text-zinc-600 text-sm ml-auto">{completedFaelle?.length ?? 0} Faelle</span>
          </div>

          {!completedFaelle?.length ? (
            <div className="bg-zinc-900 rounded-2xl p-12 text-center border border-zinc-800">
              <p className="text-zinc-500">Noch keine abgerechneten Faelle vorhanden.</p>
            </div>
          ) : (
            <>
              {/* Desktop table */}
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-zinc-800">
                        ...
                        <th className="text-right px-4 py-3 text-zinc-400 font-medium whitespace-nowrap">Schadenhoehe</th>
                        <th className="text-right px-4 py-3 text-zinc-400 font-medium whitespace-nowrap">Leadpreis</th>
                        <th className="text-left px-4 py-3 text-zinc-400 font-medium">Datum</th>
                      </tr>
                    </thead>
                    <tbody>
                      {completedFaelle.map((fall) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-zinc-200 text-right tabular-nums">{betrag}</td>
                            <td className="px-4 py-3 text-right tabular-nums">
                              {abr ? (
                                <span className={abr.preistyp === 'einzel' ? 'text-amber-400' : 'text-zinc-200'}>
                                  {leadpreisStr}
                            ...
                            <td className="px-4 py-3 text-zinc-400 text-xs whitespace-nowrap">{datum}</td>
                          </tr>
                        )
                      })}
                    </tbody>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className="text-zinc-400 text-xs">{datum}</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                        <div>
                          <span className="text-zinc-500 text-xs">Schadenhoehe</span>
                          <p className="text-zinc-200 tabular-nums">{betrag}</p>
                        </div>
                        <div className="text-right">
                          <span className="text-zinc-500 text-xs">Leadpreis</span>
                          <p className={abr ? (abr.preistyp === 'einzel' ? 'text-amber-400' : 'text-zinc-200') : 'text-zinc-500'}>
                            {leadpreisStr}
                          </p>
                        </div>
                      </div>
        ...
        {(einzahlungen?.length ?? 0) > 0 && (
          <div className="mb-8">
            <h2 className="text-lg font-semibold text-white mb-4">Einzahlungen</h2>
            <div className="bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-800">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-zinc-800">
                    <th className="text-left px-4 py-3 text-zinc-400 font-medium">Typ</th>
                    <th className="text-right px-4 py-3 text-zinc-400 font-medium">Betrag</th>
                    <th className="text-left px-4 py-3 text-zinc-400 font-medium">Beschreibung</th>
                    <th className="text-left px-4 py-3 text-zinc-400 font-medium">Datum</th>
                  </tr>
                </thead>
                <tbody>
                  {einzahlungen!.map(e => (
                    <tr key={e.id} className="border-b border-zinc-800/50">
                      <td className="px-4 py-3">
                        <span className="px-2 py-0.5 rounded-md text-xs font-medium bg-emerald-950 text-emerald-400">
                      ...
                      <td className="px-4 py-3 text-zinc-400 text-xs">{e.beschreibung ?? '—'}</td>
        ...
        <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-white font-semibold">Monatsabrechnung</h2>
              <p className="text-zinc-500 text-sm mt-0.5">PDF-Export Ihrer monatlichen Abrechnung</p>
            </div>
            <button
              disabled
              className="flex items-center gap-2 bg-zinc-800 text-zinc-500 text-sm font-medium py-2.5 px-4 rounded-xl cursor-not-allowed opacity-50"
            >
              <DownloadIcon className="w-4 h-4" />
              PDF Download
            </button>
          </div>
          <p className="text-zinc-600 text-xs mt-3">Coming soon - PDF-Abrechnungen werden in Kuerze verfuegbar sein.</p>
        </div>
      </div>
    </div>
  )
