...
**Allowlist** (sanktionierte, absichtliche Direkt-Writer — in `check-operative-status-writes.mjs`): `state-machine.ts` (DIE Engine) + `claims/endzustand-actions.ts` (dokumentierte Cursor-Ausnahme: die 2 nicht-terminalen Endzustand-Outcomes sind Cursor-Werte, die die Engine liest — `state-machine.ts:52-66`). Neue legitime Cursor-Ausnahme → Allowlist mit Begründung, **nicht** die Baseline aufblähen.

**Baseline = 4 grandfathered** (bekannte Direkt-Writer, per Boy-Scout auf den Funnel heben): `gutachter/team/actions.ts` (sv-zugewiesen), `kanzlei-wunsch/actions.ts` (an_externe_kanzlei_uebergeben / in_kommunikation_vs), `lexdrive/process-event.ts` (VS-Webhook), `termine/close-nur-gutachter-termin.ts`. Der Reparatur-Abschluss (`reparatur-abschluss-actions.ts`) war der 5. — er ist bereits auf den Funnel gehoben (`kitta/reparatur-cursor-funnel`) und fiel damit aus der Baseline. Marker: `coordination-an-status-achsen-lane-werkstatt-abschluss-bypass`.
