...
const OUT_DIR = join(PROJECT_ROOT, 'docs', '17.05.2026', 'cmm44-spa-pr2-smoke')
...
async function smokeAdmin(browser, fallId) {
  ...
  if (detailUrl) {
    ...
    // ──── KRITISCH: formerly-34-column Felder prüfen ─────────────────────────
    // Diese Felder kamen früher aus faelle-Spalten, kommen jetzt aus claims.
    // Wenn sie leer/fehlen → can_access_fall oder claims-Join kaputt.
    await page.waitForTimeout(1000)
    const bodyText = await page.locator('body').innerText().catch(() => '')

    // Polizei-Felder: suche nach Kennzeichnungen oder Sektion-Labels
    const hatPolizeiSektion = bodyText.match(/Polizei|Aktenzeichen|Polizeibericht|polizei/i) !== null
    notiere('admin', 'detail-polizei-felder', hatPolizeiSektion ? 'OK' : 'WARN',
      hatPolizeiSektion ? 'Polizei-Sektion sichtbar' : 'Kein Polizei-Sektion-Text gefunden — möglicherweise leer oder Spalte fehlt')

    // Unfallskizze-Status
    const hatUnfallskizze = bodyText.match(/Unfallskizze|unfallskizze|Skizze/i) !== null
    notiere('admin', 'detail-unfallskizze', hatUnfallskizze ? 'OK' : 'WARN',
      hatUnfallskizze ? 'Unfallskizze-Bereich sichtbar' : 'Kein Unfallskizze-Text — evtl. leer oder feld dropped')

    // Kanzlei-Ansprechpartner
    const hatKanzlei = bodyText.match(/Kanzlei|Ansprechpartner|kanzlei/i) !== null
    notiere('admin', 'detail-kanzlei-ansprechpartner', hatKanzlei ? 'OK' : 'WARN',
      hatKanzlei ? 'Kanzlei-Ansprechpartner-Block sichtbar' : 'Kein Kanzlei-Block — evtl. RLS-Problem oder kein Paket')

    // Finanzierung
    const hatFinanzierung = bodyText.match(/Finanzierung|finanzier|Leasing|leasing|finanzierung/i) !== null
    notiere('admin', 'detail-finanzierung', hatFinanzierung ? 'OK' : 'WARN',
      hatFinanzierung ? 'Finanzierung-Felder sichtbar' : 'Kein Finanzierungs-Text gefunden')

    // Gewerbe-Flag
    const hatGewerbe = bodyText.match(/Gewerbe|gewerblich|gewerbe/i) !== null
    notiere('admin', 'detail-gewerbe-flag', hatGewerbe ? 'OK' : 'WARN',
      hatGewerbe ? 'Gewerbe-Feld sichtbar' : 'Kein Gewerbe-Text gefunden')

    // KB-Zuweisung
    const hatKb = bodyText.match(/KB|Kundenbetreuer|Sachbearbeiter|zugewiesen/i) !== null
    notiere('admin', 'detail-kb-zuweisung', hatKb ? 'OK' : 'WARN',
      hatKb ? 'KB-Zuweisung sichtbar' : 'Kein KB-Zuweisungs-Text gefunden')
