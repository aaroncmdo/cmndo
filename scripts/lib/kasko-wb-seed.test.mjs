...
describe('sql', () => {
  ...
  it('buildSeedSql ist idempotent (ON CONFLICT), koppelt per slug und backfillt den Rechtstraeger per Name', () => {
    ...
    expect(sql).toContain("('huk-coburg', 'Classic', 'SELECT', 'Classic SELECT', true, 'voll', 'belegt', 40)")
    expect(sql).toContain('JOIN public.kasko_versicherer_marken m ON m.slug = t.slug')
    expect(sql).toContain("('huk-coburg', 'HUK-COBURG-Allgemeine Versicherung AG')")
    expect(sql).toContain('JOIN public.versicherungen v ON v.name = x.versicherung_name')
    expect(sql).toContain("('__default__', NULL, ")
    // kompakt: genau EINE Anweisung je Tabelle (VALUES-Liste), nicht eine je Zeile
    expect(sql.match(/INSERT INTO public\.kasko_versicherer_marken/g)).toHaveLength(1)
    expect(sql.match(/INSERT INTO public\.kasko_tarife/g)).toHaveLength(1)
    expect(sql.match(/INSERT INTO public\.kasko_wb_konditionen/g)).toHaveLength(1)
    expect(sql.match(/UPDATE public\.kasko_versicherer_marken m SET versicherung_id/g)).toHaveLength(1)
