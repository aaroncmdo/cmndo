...
import { scanRdg, scanUmlaute, scanTitleBrandTwice } from './lib/copy-lint-scan.mjs'
...
for (const f of SCAN_FILES) {
  ...
  const isGerman = !isI18n || /\/de\.json$/.test(rel)
...
console.log(`\ncopy-lint: ${findings.length} Treffer in ${files.length} Files (RDG: ${findings.filter(f => f.code.startsWith('rdg')).length}, Umlaut: ${findings.filter(f => f.code === 'umlaut').length}, Titel: ${findings.filter(f => f.code === 'title-brand-twice').length})`)
